...
# Cola CUPS: el instalador no crea colas (lpadmin necesita administrador y el
# agente corre como usuario). Si la cola ya existe (creada por el sistema al
# instalar la impresora), se informa su URI real; si no, se deja el comando.
if lpstat -p 2>/dev/null | grep -q "printer ZKP8008"; then
  ...
  echo "  El agente ya usa TCP directo; si querés el respaldo CUPS, creá la cola"
  echo "  en el puente (pide tu contraseña):"
  echo
  echo "      sudo lpadmin -p MobOS_LAN -E -v socket://${IMPRESORA}:${PUERTO} -m raw"
  echo
  echo "  Verificá después con:  lpstat -v MobOS_LAN"
fi
