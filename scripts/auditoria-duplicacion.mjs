...
const MEDICIONES = [
  ...
  // Duplicación pendiente (cuanto más bajo, mejor).
  {
    ...
    filas: [
      ...
      contar('portapapeles sin objeto', /navigator\.clipboard/g),
      contar('descarga con Blob + <a>', /createObjectURL\(new Blob/g),
      contar('wa.me a mano', /wa\.me\//g),
      contar('device-id repetido', /mobos:device-id/g),
      contar('vista list/grid sin hook', /localStorage\.(getItem|setItem)\('mobos:[a-z-]*vista/g),
      contar('monto en Gs a mano', /Gs\.?[^`"']{0,40}toLocaleString\('es-PY'\)/g),
