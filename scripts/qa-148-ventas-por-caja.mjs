...
const SALIDA = process.env.QA_OUT || join(RAIZ, 'docs/qa/148-18-ventas-por-caja')
...
// Sesión real del entorno local (seed de e2e); en producción se omite.
const loginRespuesta = SOLO_DEMO ? null : await fetch(`${API}/api/auth/login`, { method: 'POST', headers: { 'content-type': 'application/json', origin: WEB }, body: JSON.stringify({ email: 'e2e-tienda@test.local', password: 'E2e-password-123', deviceId: 'qa-148' }) })
...
if (!SOLO_DEMO) await ver('Sesión real: el corte por caja lista las sesiones del período', async () => {
