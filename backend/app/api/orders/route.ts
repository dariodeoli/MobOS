  ...
  if (discount > 0 && items.some(item => item?.couponCode !== undefined)) throw new InputError('No se puede combinar cupón y descuento global.')
    const crearPedido = () => prisma.$transaction(async tx => {
      ...
      for (const item of items) {
        ...
        let unitCostPyg: number | undefined; let baseUnitCostPyg: number | undefined; let listPricePyg: number | undefined; let priceSource: string | undefined; let insurancePyg = 0; let extraCostPyg = 0; let costPending = false; let serialsPending = 0; let stockPending = 0; let faltanteOffline = 0
        ...
        if (item.productId) {
          ...
          if (decrementBy > 0) {
            if (offlineSale) {
              ...
              faltanteOffline = decrementBy - aDescontar
              stockFaltante += faltanteOffline
            } else {
      ...
      const demandas = demandasDePedido({ orderId: order.id, items: demandaItems, promisedAt: prometida, customerId, branchId, ventaConfirmada: confirmed > 0, ahora: new Date() })
