...
export async function POST(request: Request) {
  ...
  try {
    const data = await prisma.$transaction(async tx => {
      ...
      await tx.auditLog.create({ data: { tenantId: tenant, userId: session.user.id, action: 'PRODUCT_CREATED', entity: 'Product', entityId: product.id, metadata: { name: product.name, sku: product.sku, pricePyg: product.pricePyg, costPyg: product.costPyg } } })
      return product
    })
...
export async function PATCH(request: Request) {
  ...
  try {
    const data = await prisma.$transaction(async tx => {
      ...
      await tx.auditLog.create({ data: { tenantId: tenant, userId: session.user.id, action: 'PRODUCT_UPDATED', entity: 'Product', entityId: product.id, metadata: { name: typeof b.name === 'string' && b.name.trim() ? b.name.trim() : product.name, pricePyg: price ?? product.pricePyg, costPyg: cost ?? product.costPyg, condition: b.condition ?? product.condition } } })
