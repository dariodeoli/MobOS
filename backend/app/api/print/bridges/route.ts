...
export async function POST(request: Request) {
  ...
  const puente = await prisma.$transaction(async tx => {
    const created = await tx.printBridge.create({
      data: { tenantId, name, tokenHash: hashToken(generarTokenPuente()), pairingCodeHash: codeHash, pairingExpiresAt: expiresAt, createdByUserId: session.user.id },
    })
    await tx.auditLog.create({ data: { tenantId, userId: session.user.id, action: 'PRINT_BRIDGE_CREATED', entity: 'PrintBridge', entityId: created.id, metadata: { name } } })
    return created
  })
  return json({ bridge: { id: puente.id, name: puente.name }, pairingCode: code, expiresAt }, { status: 201 })
}
