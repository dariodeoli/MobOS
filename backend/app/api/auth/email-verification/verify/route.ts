...
export async function POST(request: Request) {
  ...
  const record = await prisma.emailVerificationToken.findUnique({ where: { tokenHash: hashToken(`EMAIL_VERIFICATION:${token}`) }, select: { id: true, tenantId: true, purpose: true } })
  if (!record || record.purpose !== 'EMAIL_VERIFICATION') return safeInvalid()
  // El vencimiento lo decide Postgres, no la hora de Node.
  const verified = await prisma.$transaction(async tx => {
    const consumed = await tx.$executeRaw`UPDATE "EmailVerificationToken" SET "consumedAt" = now() WHERE "id" = ${record.id} AND "purpose" = 'EMAIL_VERIFICATION' AND "consumedAt" IS NULL AND "revokedAt" IS NULL AND "expiresAt" > now()`
