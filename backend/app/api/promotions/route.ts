...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    const promotion = await prisma.promotion.findFirst({ where: { id, tenantId: session.user.tenantId }, select: { code: true, name: true } })
    const result = await prisma.promotion.updateMany({ where: { id, tenantId: session.user.tenantId }, data: { isActive: b.isActive } })
    if (result.count) await prisma.auditLog.create({ data: { tenantId: session.user.tenantId, userId: session.user.id, action: b.isActive ? 'PROMOTION_ACTIVATED' : 'PROMOTION_DEACTIVATED', entity: 'Promotion', entityId: id, metadata: { isActive: b.isActive, code: promotion?.code, name: promotion?.name } } })
