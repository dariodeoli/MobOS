...
const porDestino = await filasAuditoria('q=10.9.9.9')
assert.ok(porDestino.some(fila => fila.action === 'PRINT_PRINTER_CREATED'), 'un destino de impresora encuentra su evento')
assert.ok(porDestino.every(fila => JSON.stringify(fila.metadata).includes('10.9.9.9') || fila.action.includes('10.9.9.9') || (fila.entityId || '').includes('10.9.9.9')), 'la búsqueda devuelve solo filas con la coincidencia')
...
const csvFiltrado = await descargar('/api/exports/audit.csv?q=10.9.9.9')
...
assert.ok(lineas.every(linea => linea.includes('10.9.9.9')), 'el CSV respeta el filtro de búsqueda')
