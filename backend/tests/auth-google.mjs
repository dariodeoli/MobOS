...
try {
  ...
  // #137: el login por PIN de una sesión de empresa acotada a sucursal acepta
  // a esa sucursal y al equipo sin sucursal, y rechaza otra sucursal.
  const bcryptjs = require('bcryptjs')
  const scopedTenant = await prisma.tenant.create({ data: { name: 'Tienda Sucursales', email: 'sucursales@example.test', passwordHash: await bcryptjs.hash('Clave-sucursales-123', 12), slug: 'tienda-sucursales' } })
  const branchCentro = await prisma.branch.create({ data: { tenantId: scopedTenant.id, name: 'Centro' } })
  const branchShopping = await prisma.branch.create({ data: { tenantId: scopedTenant.id, name: 'Shopping' } })
  const tecnico = await prisma.user.create({ data: { tenantId: scopedTenant.id, name: 'Técnico sin sucursal', role: 'TECNICO', pinHash: await bcryptjs.hash('4826', 12) } })
  const vendedorCentro = await prisma.user.create({ data: { tenantId: scopedTenant.id, name: 'Vendedor Centro', role: 'VENDEDOR', branchId: branchCentro.id, pinHash: await bcryptjs.hash('1111', 12) } })
  const vendedorShopping = await prisma.user.create({ data: { tenantId: scopedTenant.id, name: 'Vendedor Shopping', role: 'VENDEDOR', branchId: branchShopping.id, pinHash: await bcryptjs.hash('2222', 12) } })
  const scopedCompany = await auth.authenticateCompany({ email: 'sucursales@example.test', password: 'Clave-sucursales-123', deviceId: 'it-branch', branchId: branchCentro.id })
  ok(!!scopedCompany && scopedCompany.sellers.some(u => u.id === tecnico.id), 'la lista acotada a sucursal incluye al equipo sin sucursal')
  ok(scopedCompany.sellers.some(u => u.id === vendedorCentro.id) && !scopedCompany.sellers.some(u => u.id === vendedorShopping.id), 'la lista acotada no cruza de sucursal')
  ok(await auth.authenticateCompany({ email: 'sucursales@example.test', password: 'Clave-sucursales-123', deviceId: 'it-branch-ajena', branchId: 'sucursal-inexistente' }) === null, 'una sucursal inexistente no abre sesión de empresa')
  const scopedCookie = `${oauth.COOKIE_COMPANY}=${scopedCompany.accessToken}`
  await post(pin, { sellerId: tecnico.id, pin: '4826' }, scopedCookie)
  const sesionTecnico = await prisma.session.findFirst({ where: { userId: tecnico.id, level: 'SELLER' }, orderBy: { createdAt: 'desc' } })
  ok(sesionTecnico.branchId === branchCentro.id, 'la sesión del técnico hereda el alcance de la empresa')
  await post(pin, { sellerId: vendedorCentro.id, pin: '1111' }, scopedCookie)
  await post(pin, { sellerId: vendedorShopping.id, pin: '2222' }, scopedCookie, 401)
  await post(pin, { pin: '4826' }, scopedCookie)
  await post(pin, { pin: '2222' }, scopedCookie, 401)
  ...
  ok(again.data.tenant.id === tenantId && await prisma.tenant.count() === 3, 'stable sub login, no new tenant or privilege changes')
  ...
  ok(await prisma.tenant.count() === 3 + concurrentTenants.length, 'only the raced stores were created')
