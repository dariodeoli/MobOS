...
async function testHttp([base, admin, seller, company, otherSeller]) {
  ...
  const saved = (await customers()).find(c => c.id === first.customerId)
  assert.equal(saved.phone, customer.phone); assert.equal(saved.notes, `Dirección: ${customer.address}`)
  ...
  await reject({ customer: { ...customer, address: 'Different address' } })
