...
const emptyCustomer = { name: '', document: '', email: '', phones: [''], countryCode: '+595', addresses: [{ label: 'Principal', address: '', city: '', department: '', country: 'Paraguay' }], acceptsEmailMarketing: false, acceptsSmsMarketing: false, acceptsWhatsappMarketing: false, taxExempt: false, tags: '', pricingTier: 'RETAIL', creditLimitPyg: '', creditDays: '' }
...
export default function SellerCustomers() {
  ...
  async function create(event) {
    ...
    try {
      const countryCode = form.countryCode || '+595'
      const phones = form.phones.map((phone) => soloDigitos(phone)).filter(Boolean).slice(0, 5)
      if (phones.some((phone) => !telefonoValido(phone, countryCode))) throw new Error(MENSAJE_TELEFONO)
  ...
  return <SellerSection title="Clientes" description={esDemo ? 'Demo local: ingresá únicamente datos ficticios.' : 'Buscá por nombre o teléfono. La API devuelve hasta 50 coincidencias.'}>
    ...
    <Modal open={crearAbierto} onClose={() => !saving && setCrearAbierto(false)} title="Crear cliente" className="max-w-2xl">
      <form onSubmit={create} className="space-y-4">
        ...
        <fieldset className="space-y-2"><legend>Teléfonos</legend><label className="block space-y-2"><span>Código de país</span><Input className="w-28" aria-label="Código de país" inputMode="numeric" maxLength={5} disabled={saving} value={form.countryCode} placeholder="+595" onChange={(event) => setForm({ ...form, countryCode: codigoPais(event.target.value) })} /><small className="text-mute">Podés cambiarlo: +595, +55, +54…</small></label>{form.phones.map((phone, index) => <div className="flex gap-2" key={`phone-${index}`}><Input type="tel" inputMode="numeric" maxLength={15} disabled={saving} value={phone} placeholder={index === 0 ? '0981 123 456' : 'Otro teléfono'} onChange={(event) => setForm({ ...form, phones: form.phones.map((item, itemIndex) => itemIndex === index ? soloDigitos(event.target.value, 15) : item) })} />{form.phones.length > 1 && <button type="button" className="rounded-xl border border-fore/15 px-3 text-sm" onClick={() => setForm({ ...form, phones: form.phones.filter((_, itemIndex) => itemIndex !== index) })}>Quitar</button>}</div>)}{form.phones.length < 5 && <button type="button" className="text-sm font-semibold text-fono-light" onClick={() => setForm({ ...form, phones: [...form.phones, ''] })}>+ Añadir teléfono</button>}</fieldset>
