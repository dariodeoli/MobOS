...
const AUDIT_LABELS = {
  ...
  ORDER_RETURN_RECORDED: (meta) => `Devolución${meta?.reason ? `: ${meta.reason}` : ''}${meta?.refundMode === 'CREDIT' ? ' · saldo a favor del cliente' : Number(meta?.refundPyg || 0) > 0 ? ` · reembolso ${gs(Number(meta.refundPyg))}` : ''}${Number(meta?.restockedUnits || 0) > 0 ? ` · ${meta.restockedUnits} unidad(es) repuestas` : ''}`,
  ORDER_EXCHANGE_RECORDED: (meta) => `Cambio registrado${meta?.reason ? `: ${meta.reason}` : ''}${Number(meta?.restockedUnits || 0) > 0 ? ` · ${meta.restockedUnits} unidad(es) repuestas` : ''}`,
  ORDER_CANCELLED: (meta) => `Pedido cancelado${meta?.reason ? `: ${meta.reason}` : ''}${meta?.refundMode === 'CREDIT' ? ' · saldo a favor del cliente' : Number(meta?.refundPyg || 0) > 0 ? ` · reembolso ${gs(Number(meta.refundPyg))}` : ''}${Number(meta?.restockedUnits || 0) > 0 ? ` · ${meta.restockedUnits} unidad(es) repuestas` : ''}`,
  STORE_CREDIT_ISSUED: () => 'Saldo a favor emitido para el cliente',
  STORE_CREDIT_USED: (meta) => `Saldo a favor usado${Number(meta?.amountPyg || 0) > 0 ? `: ${gs(Number(meta.amountPyg))}` : ''}`,
