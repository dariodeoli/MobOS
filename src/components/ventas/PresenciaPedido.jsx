  ...
  return (
    <span className={`flex items-center gap-2 ${className}`} title={etiquetaPresencia(otros)} aria-label={etiquetaPresencia(otros)}>
      <span className="flex -space-x-2">
        {otros.slice(0, 3).map((persona) => <Avatar key={persona.id} user={persona} picture={persona.picture || persona.avatarUrl} hasAvatar={persona.hasAvatar} size="sm" className="border-paper" />)}
      </span>
      <span className="text-xs font-semibold text-ok">
        {otros.length === 1 ? `${primerNombre(otros[0].name)} está viendo este pedido` : `${otros.length} personas están viendo este pedido`}
      </span>
    </span>
  )
}
