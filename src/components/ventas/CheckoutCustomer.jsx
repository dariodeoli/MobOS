  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      try {
        ...
        if (!active) return
        const coincidencias = rows.filter(customer => coincideCliente(customer, query))
        setMatches(coincidencias.slice(0, 5)); setError('')
        // Si lo escrito coincide exacto con una única ficha, se selecciona sola:
        // así la venta toma su lista de precios sin un clic extra.
        const exactos = coincidencias.filter(customer => String(customer.name || '').trim().toLowerCase() === query.toLowerCase())
