  ...
  return <div className="space-y-3 md:col-span-2">
    ...
    <details className="rounded-xl border border-ink-600 p-3"><summary className="flex cursor-pointer items-center gap-2 text-sm font-medium text-fono-light"><Icon name="user" className="h-3.5 w-3.5" /> Datos de contacto, RUC/CI y direcciones</summary>
      <div className="mt-3 grid gap-3 sm:grid-cols-2">
        ...
        <div className="text-xs text-mute">CI o RUC<RucField disabled={false} value={value.document || ''} onChange={(document) => onChange({ ...value, document })} onAplicar={(datos) => onChange({ ...value, name: datos.name || value.name, document: datos.fullRuc || value.document })} mostrarExtractor={!esDemo} /></div>
    ...
    {onBillingChange && (
      <details className="rounded-xl border border-ink-600 p-3">
        ...
        <div className="mt-3 grid gap-3 sm:grid-cols-2">
          ...
          <div className="text-xs text-mute">RUC del titular<RucField value={billingTo?.document || ''} onChange={(document) => onBillingChange({ ...billingTo, document })} onAplicar={(datos) => onBillingChange({ ...billingTo, name: datos.name || billingTo?.name, document: datos.fullRuc || billingTo?.document })} mostrarExtractor={!esDemo} /></div>
        </div>
      </details>
    )}
  </div>
