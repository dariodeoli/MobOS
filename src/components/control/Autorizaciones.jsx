...
function resumenValor(kind, value) {
  ...
  if (kind === 'STOCK_ADJUST')
    return `${data.action === 'remove' ? 'Retirar unidad' : 'Ajustar unidad'}${data.reason ? ` · ${data.reason}` : ''}`
  const parts = []
  if (data.creditLimitPyg !== undefined && data.creditLimitPyg !== null)
    parts.push(`Límite ${formatGs(data.creditLimitPyg)}`)
  if (data.creditDays !== undefined && data.creditDays !== null)
    parts.push(`${data.creditDays} día${Number(data.creditDays) === 1 ? '' : 's'}`)
  if (data.discountPyg !== undefined && data.discountPyg !== null)
    parts.push(`Descuento ${formatGs(data.discountPyg)}`)
  if (data.maxDiscountPyg !== undefined && data.maxDiscountPyg !== null)
    parts.push(`Máximo ${formatGs(data.maxDiscountPyg)}`)
  if (data.discountPct !== undefined && data.discountPct !== null)
    parts.push(`${data.discountPct}%`)
...
export default function Autorizaciones() {
  ...
  function abrirAprobar(row) {
    const value =
      row.requestedValue && typeof row.requestedValue === 'object' ? row.requestedValue : {}
    setApproveForm({
  ...
  async function confirmarAprobar() {
    ...
    if (kind === 'DISCOUNT' || kind === 'BELOW_LIST_PRICE') {
      ...
      if (!Number.isSafeInteger(max) || max < 0 || max > DISCOUNT_MAX) {
        toast.error(
          'Máximo inválido',
          'El máximo autorizado debe ser un entero entre 0 y 100.000.000.',
        )
        return
      }
    ...
    try {
      await api.patch('/api/authorizations', {
        ...
        ...(approveForm.resolvedNote.trim()
          ? { resolvedNote: approveForm.resolvedNote.trim() }
          : {}),
      })
      toast.success(
        'Solicitud aprobada',
        kind === 'WHOLESALE' || kind === 'CREDIT' || kind === 'CREDIT_DAYS' || kind === 'DISCOUNT'
    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        {!loading && !error && !rows.length && (
          <EmptyState
            ...
            description="Cuando un vendedor pida mayorista, crédito, plazo, descuento, precio bajo lista, ajuste de stock o anulación, aparecerá acá."
          />
        )}
        <div className="overflow-x-auto" data-testid="autorizaciones-tabla">
          ...
          <div className="space-y-1">
            {rows.map(row => {
              ...
              // El dueño resuelve también lo que pidió él (trabaja solo); un
              // vendedor o gerente no: su solicitud la resuelve otra persona.
              const puedeResolverEsta = puedeResolver && (!propia || usuario?.role === 'ADMIN')
              const pedido = resumenValor(row.kind, row.requestedValue)
              const autorizado =
                row.status === 'APPROVED'
                  ? resumenValor(row.kind, row.resolvedValue || row.requestedValue)
                  : row.status === 'PENDING'
                    ? ''
                    : 'rechazado'
              const detalle = [
                ...
                row.resolvedBy?.name
                  ? `Resolvió ${row.resolvedBy.name} · ${fechaHora(row.resolvedAt)}`
                  : '',
              ...
              return (
                <div
                  key={row.id}
                  data-testid="autorizacion-fila"
                  className={cn(
                    GRID_AUTORIZACIONES,
                    'rounded-xl border px-3.5 py-2 transition',
                    row.status === 'PENDING'
                ...
                >
                  ...
                  <span className="flex items-center gap-1">
                    <Badge
                      color="blue"
                      className="w-fit justify-self-start whitespace-nowrap px-1.5 py-0.5 text-[10px]"
                    >
                    ...
                    {row.usedAt && (
                      <Badge
                        color="slate"
                        className="w-fit whitespace-nowrap px-1.5 py-0.5 text-[10px]"
                      >
                        Usada
                      </Badge>
                    )}
                  ...
                  <span className="flex items-center gap-1">
                    ...
                    {propia && row.status === 'PENDING' && (
                      <span className="truncate text-[10px] text-mute">Tu solicitud</span>
                    )}
                  </span>
                  <span className="flex items-center justify-end gap-1">
                    {puedeResolverEsta && row.status === 'PENDING' && (
                      <>
                        <Button
                          type="button"
                          className="h-8 px-2 text-xs"
                          onClick={() => abrirAprobar(row)}
                        >
                          Aprobar
                        </Button>
                        <Button
        ...
        {approveTarget && (
          <div className="space-y-4">
            <p className="text-sm text-mute">
              {approveTarget.customer?.name || sujetoDe(approveTarget) || 'Cliente'} · pedido por{' '}
              {approveTarget.requestedBy?.name || 'Sistema'}:{' '}
              <b className="text-fore">
                {resumenValor(approveTarget.kind, approveTarget.requestedValue)}
              </b>
            </p>
            ...
            {approveTarget.kind === 'CREDIT' && (
              <FormField label="Límite de crédito autorizado (Gs.)" htmlFor="auth-limit">
                <MoneyInput
                  id="auth-limit"
                  value={approveForm.creditLimitPyg}
                  onValueChange={value =>
                    setApproveForm(form => ({ ...form, creditLimitPyg: value }))
                  }
            ...
            {(approveTarget.kind === 'DISCOUNT' || approveTarget.kind === 'BELOW_LIST_PRICE') && (
              ...
              >
                <MoneyInput
                  id="auth-discount"
                  value={approveForm.maxDiscountPyg}
                  onValueChange={value =>
                    setApproveForm(form => ({ ...form, maxDiscountPyg: value }))
                  }
                  placeholder="50.000"
                />
              </FormField>
            ...
            {(approveTarget.kind === 'CREDIT' || approveTarget.kind === 'CREDIT_DAYS') && (
              <FormField
                label={
                  approveTarget.kind === 'CREDIT_DAYS'
                    ? 'Días autorizados (máximo)'
                    : 'Días de crédito'
                }
                hint={
                  approveTarget.kind === 'CREDIT_DAYS'
                    ? 'Podés autorizar menos días de los pedidos: es el máximo habilitado.'
                    : undefined
                }
                htmlFor="auth-days"
              >
                <Input
                  ...
                  onChange={event =>
                    setApproveForm(form => ({ ...form, creditDays: event.target.value }))
                  }
                  placeholder="30"
                />
              </FormField>
            ...
            <FormField label="Nota de la respuesta (opcional)" htmlFor="auth-note">
              <Textarea
                ...
                rows={2}
                maxLength={500}
                value={approveForm.resolvedNote}
                onChange={event =>
                  setApproveForm(form => ({ ...form, resolvedNote: event.target.value }))
                }
      ...
      >
        {rejectTarget && (
          <div className="space-y-4">
            <p className="text-sm text-mute">
              {rejectTarget.customer?.name || sujetoDe(rejectTarget) || 'Cliente'} · pedido por{' '}
              {rejectTarget.requestedBy?.name || 'Sistema'}:{' '}
              <b className="text-fore">
