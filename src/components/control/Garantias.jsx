  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><Eyebrow>Servicio y seguimiento</Eyebrow><h2 className="mt-1 text-2xl font-bold tracking-tight">Garantías</h2><p className="mt-1 text-sm text-mute">Seguimiento por caso, sin inventar cobertura automática.</p></div><div className="relative w-full max-w-sm"><Icon name="search" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-mute" /><Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar cliente, serial o detalle…" className="pl-9" aria-label="Buscar garantías" /></div></div>
    ...
    <div className="space-y-2">{visible.map((item) => <article key={item.id} className="rounded-xl border border-ink-600 px-3 py-2.5 transition hover:border-fono/40">
      <div className="flex flex-wrap items-center gap-2">
        ...
        <span className="ml-auto flex shrink-0 flex-wrap items-center gap-1.5">{telefonoDelCaso(item) && <WhatsAppMenu telefono={telefonoDelCaso(item)} countryCode={item.customerCountryCode || item.customer?.countryCode || '+595'} category="SERVICE" title={item.customerName} contexto={{ cliente: item.customerName || '', nombre: item.customerName || '', equipo: item.serial || '', servicio: item.description || '', estado: label[item.status] || '', fecha: item.createdAt ? new Date(item.createdAt).toLocaleDateString('es-PY') : '' }} />}{item.publicToken && <Button type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => { navigator.clipboard?.writeText(`${window.location.origin}/garantia/${item.publicToken}`).catch(() => {}); toast.success('Enlace de garantía del cliente copiado.') }}>Página cliente</Button>}{!esDemo && <Button type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => abrirFotos(item)}>Fotos</Button>}{item.status !== 'DELIVERED' && <Button type="button" variant="outline" className="h-8 px-2 text-xs" disabled={advancingId !== null} onClick={() => advance(item)}>{advancingId === item.id ? 'Actualizando…' : 'Avanzar'}</Button>}</span>
      </div>
