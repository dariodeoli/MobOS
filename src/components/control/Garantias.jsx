...
export default function Garantias() {
  ...
  // Recepción de repuestos con el buscador dependiente (#250): producto o modelo
  // compatible → marca y categoría; lo elegido se agrega a la lista de repuestos.
  ...
  // Agrega el repuesto elegido a la lista (una línea por repuesto, como siempre);
  // el texto sigue siendo el que viaja a la base y a la ficha del caso.
  function agregarRepuesto() {
    const etiqueta = etiquetaDispositivo(repuesto)
    if (!etiqueta) return
    const lineas = String(form?.partsText || '').split('\n').map((linea) => linea.trim()).filter(Boolean)
    if (!lineas.includes(etiqueta)) lineas.push(etiqueta)
    setForm((current) => ({ ...current, partsText: lineas.join('\n') }))
    setRepuesto({})
