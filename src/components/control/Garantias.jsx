...
export default function Garantias() {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-end justify-between gap-3"><div><Eyebrow>Servicio y seguimiento</Eyebrow><h2 className="mt-1 text-2xl font-bold tracking-tight">Garantías</h2><p className="mt-1 text-sm text-mute">Seguimiento por caso, sin inventar cobertura automática.</p></div><div className="relative w-full max-w-sm"><Icon name="search" className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-mute" /><Input value={q} onChange={(e) => setQ(e.target.value)} placeholder="Buscar cliente, serial o detalle…" className="pl-9" aria-label="Buscar garantías" /></div></div>
    <Card><form onSubmit={create} className="grid gap-3 md:grid-cols-2"><div><Label htmlFor="cliente">Cliente</Label><Input id="cliente" required value={form.customerName} onChange={(e) => setForm({ ...form, customerName: e.target.value })} placeholder="Nombre del cliente" /></div><div><Label htmlFor="serial-imei">Serial / IMEI</Label><SerialField id="serial-imei" required value={form.serial} onChange={(value) => setForm({ ...form, serial: value })} placeholder="Serial o IMEI" /></div><div className="md:col-span-2"><Label htmlFor="descripcion-del-caso">Descripción del caso</Label><Textarea id="descripcion-del-caso" required rows={2} value={form.description} onChange={(e) => setForm({ ...form, description: e.target.value })} placeholder="Falla reportada, revisión solicitada…" /></div><div><Label htmlFor="responsable">Responsable</Label><Input id="responsable" value={form.responsibleName} onChange={(e) => setForm({ ...form, responsibleName: e.target.value })} placeholder="Persona responsable" /></div><div><Label htmlFor="tecnico-asignado">Técnico asignado</Label><Input id="tecnico-asignado" value={form.technicianName} onChange={(e) => setForm({ ...form, technicianName: e.target.value })} placeholder="Técnico responsable" /></div><div className="md:col-span-2"><Label htmlFor="diagnostico-inicial">Diagnóstico inicial</Label><Textarea id="diagnostico-inicial" rows={2} value={form.diagnosis} onChange={(e) => setForm({ ...form, diagnosis: e.target.value })} placeholder="Pruebas, causa probable y condición de recepción…" /></div><div><Label htmlFor="repuestos-uno-por-linea">Repuestos (uno por línea)</Label><Textarea id="repuestos-uno-por-linea" rows={2} value={form.partsText} onChange={(e) => setForm({ ...form, partsText: e.target.value })} placeholder="Pantalla OLED\nBatería" /></div><div><Label htmlFor="fotos-enlaces-uno-por-linea">Fotos / enlaces (uno por línea)</Label><Textarea id="fotos-enlaces-uno-por-linea" rows={2} value={form.photosText} onChange={(e) => setForm({ ...form, photosText: e.target.value })} placeholder="https://…" /></div><div><Label htmlFor="dias-de-garantia">Días de garantía</Label><Input id="dias-de-garantia" inputMode="numeric" value={form.warrantyDays} onChange={(e) => setForm({ ...form, warrantyDays: e.target.value.replace(/\D/g, '') })} placeholder="Ej. 90" /></div><div><Label htmlFor="vencimiento">Vencimiento</Label><Input id="vencimiento" type="date" value={form.expiresAt} onChange={(e) => setForm({ ...form, expiresAt: e.target.value })} /></div><div className="md:col-span-2"><Label htmlFor="que-cubre-una-por-linea">Qué cubre (una por línea)</Label><Textarea id="que-cubre-una-por-linea" rows={2} value={form.coverage} onChange={(e) => setForm({ ...form, coverage: e.target.value })} placeholder={'Defectos de fábrica\nPantalla y batería'} /></div><div className="md:col-span-2"><Label htmlFor="que-no-cubre-una-por-linea">Qué no cubre (una por línea)</Label><Textarea id="que-no-cubre-una-por-linea" rows={2} value={form.exclusions} onChange={(e) => setForm({ ...form, exclusions: e.target.value })} placeholder={'Daños por agua\nReparaciones de terceros'} /></div><Button type="submit" disabled={saving} className="md:col-span-2 min-h-11">{saving ? 'Guardando…' : 'Registrar caso'}</Button></form></Card>
