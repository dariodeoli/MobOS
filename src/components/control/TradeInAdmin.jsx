    ...
    <div className="space-y-4">
      ...
      <Card>
        ...
        <p className="text-sm text-mute mb-3">
          Se actualiza solo cada 4 horas desde{' '}
          {cfg.exchangeSource?.replace(' (auto)', '') || 'la casa de cambio'}. Podés sumarle un
          ajuste propio o editar el valor a mano cuando quieras.
        </p>
        ...
        <div className="flex flex-wrap items-center gap-2 mb-3 text-xs">
          ...
          {cfg.exchangeMarket > 0 && (
            <span className="text-mute">Mercado: {gs(cfg.exchangeMarket)}</span>
          )}
        </div>
        ...
        {aviso && (
          <div
            className={
              ...
              (aviso.ok ? 'bg-ok/10 border-ok/25 text-ok' : 'bg-bad/10 border-bad/25 text-bad')
            }
          >
            {aviso.texto}
          </div>
      ...
      <Card>
        ...
        <p className="text-sm text-mute mb-4">
          Valor en dólares de un equipo <b>nuevo / como nuevo</b> por capacidad. A partir de acá se
          aplican los multiplicadores de arriba.
        </p>

        <form
          onSubmit={agregarDev}
          className="grid grid-cols-1 md:grid-cols-[1fr,1fr,auto] gap-2 items-end mb-4 bg-ink-700 rounded-xl p-3"
        >
          <div>
            <Label>Modelo</Label>
            <Input
        ...
        <div className="space-y-3">
          {cfg.devices.map((d, idx) => (
            <div key={d.model} className="rounded-xl border border-ink-600 p-3">
              <div className="flex items-center justify-between mb-2">
                <div className="font-bold text-sm">{d.model}</div>
                <button
                  onClick={() => borrarDev(idx)}
                  className="text-mute hover:text-bad p-1.5"
