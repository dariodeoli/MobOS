...
import { Badge, Button, Card, EmptyState, IconAction, Input, Label, Modal, MoneyInput, Select, Skeleton, Textarea, useToast } from '@/components/ui'
...
<Modal open={suppliersOpen} onClose={() => { setSuppliersOpen(false); setEditingSupplier(null); setSupplierForm(null); setHistorySupplier(null) }} title="Proveedores" className="max-w-2xl">
      ...
      {suppliers.length === 0 ? <EmptyState compact icon="users" title="Sin proveedores registrados." /> : <div className="overflow-x-auto" data-testid="proveedores-tabla"><div className={cn(GRID_PROVEEDORES, 'px-3.5 pb-2 pt-1')}><span className={CELDA}>Proveedor</span><span className={CELDA}>Código</span><span className={CELDA}>Ciudad</span><span className={CELDA}>Teléfono</span><span className={cn(CELDA, 'text-right')}>Acciones</span></div><div className="space-y-1">{suppliers.map(item => <div key={item.id} data-testid="proveedor-fila" className={cn(GRID_PROVEEDORES, 'rounded-xl border border-ink-600 bg-ink-800/40 px-3.5 py-2')}><span className="truncate text-[13px] font-semibold" title={item.name}>{item.name}</span><span className="truncate font-mono text-[11px] text-fono-light">{item.code || '—'}</span><span className="truncate text-xs text-mute" title={item.city || undefined}>{item.city || '—'}</span><span className="truncate text-xs tabular-nums text-mute">{item.phone || '—'}</span><span className="flex items-center justify-end gap-1"><IconAction icon="clock" label={`Historial de ${item.name}`} onClick={() => setHistorySupplier(item)} /><IconAction icon="edit" tone="fono" label={`Editar ${item.name}`} onClick={() => openSupplierEdit(item)} /></span></div>)}</div></div>}
    </Modal>
