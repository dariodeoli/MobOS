...
export default function Compras() {
  ...
  return <div className="space-y-4">
    ...
    <Modal open={suppliersOpen} onClose={() => { setSuppliersOpen(false); setEditingSupplier(null); setSupplierForm(null) }} title="Proveedores">
      {editingSupplier && supplierForm && <form onSubmit={saveSupplier} className="mb-4 space-y-3 rounded-xl border border-ink-600 p-3"><p className="text-sm font-semibold">Editar proveedor</p><div className="grid gap-2 sm:grid-cols-2">{SUPPLIER_FIELDS.map(([key, label]) => key === 'city' ? <div key={key}><Label htmlFor="label">{label}</Label><CityAutocomplete id="label" value={supplierForm[key]} onSelect={(city, department) => setSupplierForm(current => ({ ...current, city, department }))} /></div> : key === 'phone' ? <div key={key}><Label htmlFor="label-2">{label}</Label><PhoneField id="label-2" disabled={busy} countryCode={supplierForm.countryCode || '+595'} phone={supplierForm.phone || ''} onCountryCodeChange={(countryCode) => setSupplierForm(current => ({ ...current, countryCode }))} onChange={(phone) => setSupplierForm(current => ({ ...current, phone }))} /></div> : key === 'email' ? <div key={key}><Label htmlFor="label-3">{label}</Label><EmailField id="label-3" disabled={busy} value={supplierForm.email || ''} onChange={(email) => setSupplierForm(current => ({ ...current, email }))} /></div> : <div key={key}><Label htmlFor="label-4">{label}</Label><Input id="label-4" required={key === 'name'} value={supplierForm[key]} onChange={(e) => setSupplierForm(current => ({ ...current, [key]: e.target.value }))} /></div>)}</div><div className="flex flex-wrap gap-2"><Button type="submit" disabled={busy}>Guardar proveedor</Button><Button type="button" variant="ghost" onClick={() => { setEditingSupplier(null); setSupplierForm(null) }}>Cancelar</Button></div></form>}
    ...
    <Modal open={paymentPurchase !== null} onClose={() => setPaymentPurchase(null)} title="Registrar pago a proveedor"><form onSubmit={paySupplier} className="space-y-3"><p className="text-sm text-mute">{paymentPurchase?.supplierName} · saldo actual {gs(paymentPurchase?.outstandingPyg || 0)}. La cotización queda congelada en este pago.</p><div className="grid grid-cols-2 gap-2"><Button type="button" variant={payment.action === 'advance' ? 'primary' : 'outline'} onClick={() => setPayment(data => ({ ...data, action: 'advance' }))}>Registrar anticipo</Button><Button type="button" variant={payment.action === 'pay' ? 'primary' : 'outline'} onClick={() => setPayment(data => ({ ...data, action: 'pay' }))}>Registrar pago</Button></div>{supplierBalance && <div className="rounded-xl border border-ink-600 p-3 text-sm"><p className="font-semibold">{supplierBalance.supplier?.name || paymentPurchase?.supplierName}</p><div className="mt-2 grid grid-cols-3 gap-2 text-xs text-mute"><span>Comprado<strong className="mt-0.5 block text-fore">{gs(supplierBalance.totalPurchasedPyg)}</strong></span><span>Pagado<strong className="mt-0.5 block text-ok">{gs(supplierBalance.paidPyg)}</strong></span><span>Pendiente<strong className="mt-0.5 block text-warn">{gs(supplierBalance.outstandingPyg)}</strong></span></div></div>}<div><Label htmlFor="cuenta-de-salida">Cuenta de salida</Label><Select id="cuenta-de-salida" required value={payment.accountId} onChange={(event) => { const account = accounts.find(item => item.id === event.target.value); setPayment(data => ({ ...data, accountId: event.target.value, currency: account?.currency || 'PYG', exchangeRatePyg: account?.currency === 'PYG' ? '1' : '' })) }}><option value="">Elegí una cuenta</option>{accounts.map(account => <option key={account.id} value={account.id}>{account.name} · {account.currency}</option>)}</Select></div><div className="grid gap-3 sm:grid-cols-2"><div><Label htmlFor="monto-en-payment-currency">Monto en {payment.currency}</Label><MoneyInput id="monto-en-payment-currency" required currency={payment.currency} value={payment.originalAmount} onValueChange={(value) => setPayment(data => ({ ...data, originalAmount: value }))} placeholder="0" /></div><div><Label htmlFor="cotizacion-en-gs">Cotización en Gs.</Label><MoneyInput id="cotizacion-en-gs" required disabled={payment.currency === 'PYG'} currency="USD" symbol="Gs." value={payment.exchangeRatePyg} onValueChange={(value) => setPayment(data => ({ ...data, exchangeRatePyg: value }))} placeholder={payment.currency === 'PYG' ? '1' : 'Ej. 7500'} /></div></div><div><Label htmlFor="referencia-pago">Referencia <span className="text-mute">(opcional)</span></Label><Input id="referencia-pago" maxLength={200} value={payment.reference} onChange={(event) => setPayment(data => ({ ...data, reference: event.target.value }))} placeholder="Transferencia, recibo o comprobante" /></div><Button className="w-full" type="submit" disabled={busy || !payment.accountId || !Number(payment.originalAmount) || !Number(payment.exchangeRatePyg)}>{payment.action === 'advance' ? 'Guardar anticipo' : 'Guardar pago'}</Button></form></Modal>
  </div>
}
