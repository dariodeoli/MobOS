...
export default function Config() {
  ...
  function cambiar(e) {
    e.preventDefault()
    if (nueva.length < 4)
      return setMsg({ tipo: 'err', txt: 'La nueva clave debe tener al menos 4 caracteres.' })
    if (nueva !== conf) return setMsg({ tipo: 'err', txt: 'Las contraseñas nuevas no coinciden.' })
    if (!cambiarClavePanel(actual, nueva))
      return setMsg({ tipo: 'err', txt: 'La contraseña actual es incorrecta.' })
    setActual('')
    setNueva('')
    setConf('')
  ...
  return (
    <div className="space-y-4">
      ...
      <Card className="bg-fono/10 border-fono/20">
        <h3 className="font-bold mb-1">Tiempo real entre celulares</h3>
        <p className="text-sm text-mute">
          Hoy los datos se guardan en este dispositivo. Para que cada vendedor cargue desde su
          propio celular y veas todo en vivo, hay que conectar <strong>Supabase</strong> (gratis).
          Avisá cuando quieras y lo activamos — solo se reescribe la capa de datos, el resto de la
          app queda igual.
        </p>
      </Card>
    </div>
