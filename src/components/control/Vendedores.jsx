...
// Roles que el dueño puede asignar desde la ficha (el propio dueño no se cambia acá).
const ROLES_CAMBIABLES = ['GERENTE', 'VENDEDOR', 'CAJERA', 'TECNICO']
...
export default function Vendedores({ seccion = 'equipo' }) {
  ...
  async function invitar(event) {
    ...
    try {
      ...
      const mensaje = cause?.message || 'No se pudo enviar la invitación.'
      // Una invitación activa se resuelve viendo la lista (reenviar o revocar).
      if (/ya existe una invitación/i.test(mensaje)) setError(mensaje + ' Mirá la lista de invitaciones para reenviarla o revocarla.')
      else setError(mensaje)
      if (/ya existe una invitación/i.test(mensaje)) cargarInvitaciones().catch(() => {})
    }
    finally { setBusy(false) }
  }
...
export function SeccionComisiones() {
