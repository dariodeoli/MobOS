    ...
    <div className="space-y-4">
      <Card>
        ...
        <form onSubmit={guardar} className="grid grid-cols-1 md:grid-cols-2 gap-3">
          <div>
            <Label>Monto ₲</Label>
            <Input inputMode="numeric" value={f.monto} onChange={set('monto')} placeholder="Ej: 250000" />
          </div>
          <div>
            <Label>Fecha</Label>
          ...
          <div className="md:col-span-2">
            <Label>Motivo</Label>
            <Input value={f.motivo} onChange={set('motivo')} placeholder="Ej: Compra de mercadería, alquiler…" autoCapitalize="sentences" />
          </div>
          <div>
      ...
      <Card className="p-0 overflow-hidden">
        ...
        {gastos.length === 0 ? (
          <div className="p-8 text-center text-slate-400 text-sm">Sin gastos registrados.</div>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 p-4">
            {gastos.map((g) => (
              <div key={g.id} className="rounded-xl border border-slate-200 p-3 flex flex-col gap-2">
                <div className="flex items-start justify-between gap-1">
                  <div className="font-semibold text-sm min-w-0 truncate">{g.motivo}</div>
                  <button onClick={() => deleteGasto(g.id)} className="text-slate-400 hover:text-bad p-1 shrink-0" title="Eliminar">🗑️</button>
                </div>
