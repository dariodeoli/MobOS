...
export default function Precios() {
  ...
  const [editor, setEditor] = useState(null) // { id?, name, currency, isActive, items }
  ...
  function abrirLista(lista = null) {
    setError('')
    setEditor(lista
      ? { id: lista.id, name: lista.name, currency: lista.currency || 'PYG', isActive: lista.isActive !== false, items: (lista.items || []).map(item => ({ tipo: item.productId ? 'PRODUCT' : 'CATEGORY', productId: item.productId || '', category: item.category || '', adjustment: item.adjustment || 'DISCOUNT', valuePct: formatPercent(item.valuePct) })) }
      : { id: null, name: '', currency: 'PYG', isActive: true, items: [] })
  }
  ...
  async function guardarLista(event) {
    ...
    try {
      const payload = { name: editor.name.trim(), currency: editor.currency, isActive: editor.isActive, items }
      ...
      {cargando ? <p className="text-sm text-mute">Cargando listas…</p> : !listas.length ? <EmptyState compact icon="store" title="Todavía no hay listas de precios." /> : <div className="space-y-2">
        {listas.map(lista => <div key={lista.id} data-testid="lista-precio-fila" className="flex flex-wrap items-center justify-between gap-2 rounded-xl border border-ink-600 bg-ink-800/40 p-3">
          <div className="min-w-0">
            <p className="flex flex-wrap items-center gap-2 text-sm font-semibold"><span className="truncate">{lista.name}</span><Badge color={lista.isActive ? 'green' : 'slate'}>{lista.isActive ? 'Activa' : 'Inactiva'}</Badge><Badge color="slate">{lista.currency}</Badge></p>
    ...
    <Modal open={editor !== null} onClose={() => !busy && setEditor(null)} title={editor?.id ? 'Editar lista de precios' : 'Nueva lista de precios'} className="max-w-3xl">
      {editor && <form onSubmit={guardarLista} className="space-y-4">
        <div className="grid gap-3 sm:grid-cols-2">
          <div><Label htmlFor="lista-nombre">Nombre</Label><Input id="lista-nombre" required value={editor.name} onChange={event => setEditor(current => ({ ...current, name: event.target.value }))} placeholder="Mayorista VIP, Empresas…" /></div>
          <div><Label htmlFor="lista-moneda">Moneda</Label><CurrencySelect id="lista-moneda" value={editor.currency} onChange={value => setEditor(current => ({ ...current, currency: value }))} /></div>
        </div>
