...
<Money value={parseGsInput(counted)} /></Button></Card>}{!esDemo && cash?.id && <Card className="space-y-3"><AttachmentList entity="CASH_SESSION" entityId={cash.id} puedeSubir={!abierta} titulo="Foto del arqueo" /><Button type="button" variant="outline" className="w-full" onClick={() => setCronologia(true)}>Cronología de la sesión</Button></Card>}<Modal open={cronologia} onClose={() => setCronologia(false)} title="Cronología de la sesión de caja">{cash?.id && <Cronologia endpoint={`/api/cash/sessions/${cash.id}/history`} active={cronologia} vacio="Sin actividad" descripcionVacio="La apertura, los movimientos, el cierre y el arqueo de esta sesión aparecerán acá." />}</Modal><AuditoriaMedios /></div>
}
