...
import { Button, Card, Eyebrow, Input, Label, Money, MoneyInput, Skeleton } from '@/components/ui'
...
export default function Caja() {
  ...
  return <div className="space-y-6"><div><Eyebrow>Finanzas</Eyebrow><h2 className="mt-2 text-2xl font-bold tracking-tight">Caja y control financiero</h2><p className="mt-1 text-sm text-mute">Apertura física en Gs., saldos, pendientes, cheques y margen con costos congelados.</p></div>{error && <p role="alert" className="rounded-lg border border-bad/30 bg-bad/10 p-3 text-sm text-bad">{error}</p>}<div className="grid gap-4 md:grid-cols-3"><Card className={abierta ? 'border-ok/25 bg-gradient-to-br from-ok/10 to-transparent' : ''}><p className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-mute">Estado</p><strong className={`flex items-center gap-2 ${abierta ? 'text-ok' : 'text-mute'}`}><span className={`h-2 w-2 rounded-full ${abierta ? 'bg-ok' : 'bg-mute'}`} />{abierta ? 'Abierta' : 'Cerrada'}</strong><p className="mt-2 text-xs text-mute">{cash?.openedAt ? new Date(cash.openedAt).toLocaleString('es-PY') : 'Sin apertura'}</p></Card><Card><p className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-mute">Saldo esperado</p><strong className="text-xl tabular-nums"><Money value={expected} /></strong><p className="mt-2 text-xs text-mute">Apertura + efectivo confirmado en Gs</p></Card><Card className={difference === 0 ? '' : 'border-warn/25 bg-gradient-to-br from-warn/10 to-transparent'}><p className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-mute">Diferencia</p><strong className={`text-xl tabular-nums ${difference === 0 ? 'text-ok' : 'text-warn'}`}><Money value={difference} /></strong><p className="mt-2 text-xs text-mute">Se calcula al cierre</p></Card></div>{finance && <div className="grid gap-4 md:grid-cols-4"><Card><p className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-mute">Por cobrar</p><strong><Money value={finance.receivables?.totalPyg || 0} /></strong></Card><Card><p className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-mute">Por pagar</p><strong><Money value={finance.payables?.totalPyg || 0} /></strong></Card><Card><p className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-mute">Margen real</p><strong><Money value={finance.margin?.profitPyg || 0} /></strong><p className="mt-1 text-xs text-mute">{finance.margin?.marginPct ?? '—'}% · seguro y extras incluidos</p></Card><Card><p className="mb-1.5 block text-[11px] font-medium uppercase tracking-wider text-mute">Cheques pendientes</p><strong>{finance.movements?.filter(m => m.kind === 'CHEQUE' && m.status === 'PENDING').length || 0}</strong></Card></div>}{!abierta ? <Card><h3 className="font-bold">Abrir caja</h3><p className="mt-1 text-sm text-mute">Registrá el fondo inicial de esta sucursal.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><div><Label htmlFor="opening">Fondo inicial (Gs)</Label><MoneyInput id="opening" value={opening} onValueChange={setOpening} placeholder="500.000" /></div><div><Label htmlFor="opening-notes">Nota</Label><Input id="opening-notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Turno mañana" /></div></div><Button className="mt-5" onClick={abrir} disabled={saving}>Abrir caja</Button></Card> : <Card><h3 className="font-bold">Cerrar caja</h3><p className="mt-1 text-sm text-mute">Contá únicamente el efectivo físico en guaraníes. No incluyas dólares, transferencias ni tarjetas.</p><div className="mt-5 grid gap-4 md:grid-cols-2"><div><Label htmlFor="counted">Efectivo contado (Gs)</Label><Input id="counted" inputMode="numeric" value={formatGsInput(counted)} onChange={(e) => setCounted(formatGsInput(e.target.value))} placeholder="0" /></div><div><Label htmlFor="close-notes">Nota de cierre</Label><Input id="close-notes" value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Observación opcional" /></div></div><Button variant="success" className="mt-5" onClick={cerrar} disabled={saving || !counted}>Cerrar caja ·
<Money value={parseGsInput(counted)} /></Button></Card>}<AuditoriaMedios /></div>
}
