...
// Tono de hover/active por acción de fila: el clic intensifica el fondo.
const ACTION_TONE = {
  ok: 'hover:bg-ok/10 hover:text-ok active:bg-ok/20',
  warn: 'hover:bg-warn/10 hover:text-warn active:bg-warn/20',
  fono: 'hover:bg-fono/10 hover:text-fono-light active:bg-fono/20',
  mute: 'hover:bg-fore/10 hover:text-mute active:bg-fore/20',
  bad: 'hover:bg-bad/10 hover:text-bad active:bg-bad/20',
}
function UnitAction({ label, icon, tone, ...props }) {
  return (
    <button
      type="button"
      title={label}
      aria-label={label}
      className={cn(
        'grid h-7 w-7 shrink-0 place-items-center rounded-lg text-fore transition',
        'active:scale-95 sm:h-8 sm:w-8',
        'disabled:cursor-not-allowed disabled:opacity-40 disabled:hover:bg-transparent disabled:hover:text-fore',
        ACTION_TONE[tone],
      )}
      {...props}
    >
...
const CONDITION_RANK = { NEW: 0, REFURBISHED: 1, USED: 2 }
const productName = unit => String(unit.product?.name || '').toLowerCase()
const compareByName = (a, b) => productName(a).localeCompare(productName(b), 'es', { sensitivity: 'base' })
// Ordena la copia de la lista ya cargada: nunca dispara nuevas llamadas.
const sortUnits = (list, key) => {
  const copy = [...list]
  switch (key) {
    case 'modelo-az':
    case 'mezclado': copy.sort(compareByName); break
    case 'modelo-za': copy.sort((a, b) => compareByName(b, a)); break
    case 'nuevos': copy.sort((a, b) => (CONDITION_RANK[a.condition] ?? 3) - (CONDITION_RANK[b.condition] ?? 3) || compareByName(a, b)); break
    case 'semis': copy.sort((a, b) => (a.condition === 'NEW' ? 1 : 0) - (b.condition === 'NEW' ? 1 : 0) || compareByName(a, b)); break
    default: copy.sort((a, b) => new Date(b.updatedAt || 0) - new Date(a.updatedAt || 0))
  }
  return copy
}
...
export default function Inventario() {
  ...
  const sortedUnits = useMemo(() => sortUnits(units, sortKey), [units, sortKey])
  ...
  return <div className="space-y-4"><Card className="p-4 md:p-5"><div className="flex flex-col gap-3 border-b border-ink-600 pb-4 md:flex-row md:items-start md:justify-between"><div><h2 className="font-bold">Inventario operativo</h2><p className="mt-1 text-sm text-mute">Cada IMEI es una unidad física con sucursal, ubicación, estado y auditoría.</p></div><div className="flex flex-wrap gap-2"><Button onClick={() => setReceiveOpen(true)}>+ Recibir unidad</Button><Button variant="outline" onClick={() => setReserveOpen(true)}>Reservar</Button><Button variant="outline" onClick={() => setTransferOpen(true)}>Transferir</Button></div></div><form onSubmit={search} className="mt-4 flex gap-2"><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Escanear IMEI, SKU o buscar modelo" autoCapitalize="characters" /><Button type="button" variant="outline" onClick={() => setScannerOpen(true)}>Escanear</Button><Button type="button" variant="outline" onClick={startCount}>Conteo rápido</Button><Button type="submit" variant="outline" disabled={busy}>Buscar</Button></form><div className="mt-4 flex gap-1 overflow-x-auto rounded-lg border border-ink-600 bg-ink-800 p-1">{[['unidades', `Unidades (${units.length})`], ...(canViewAlerts ? [['alertas', `Alertas (${(stockAlerts.alerts?.length || 0) + (stockAlerts.outOfStock?.length || 0)})`]] : []), ['reservas', `Reservas (${reservations.length})`], ['traslados', `Traslados (${transfers.length})`], ['ubicaciones', `Ubicaciones (${locations.length})`], ['compartido', 'Compartido'], ['eliminados', `Eliminados (${removedUnits.length})`]].map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`shrink-0 rounded-md px-3 py-2 text-xs font-semibold ${tab === key ? 'bg-fono/15 text-fono-light' : 'text-mute hover:text-fore'}`}>{label}</button>)}</div>{notice && <p className="mt-3 rounded-lg border border-ok/30 bg-ok/10 px-3 py-2 text-sm text-ok">{notice}</p>}{error && <p className="mt-3 rounded-lg border border-bad/30 bg-bad/10 px-3 py-2 text-sm text-bad">{error}</p>}
    {tab === 'unidades' && <div className="mt-4 space-y-2"><div className="flex flex-wrap items-center justify-end gap-2"><label htmlFor="orden-unidades" className="text-xs font-semibold text-mute">Ordenar</label><Select id="orden-unidades" value={sortKey} onChange={event => setSortKey(event.target.value)} className="w-44 shrink-0 md:h-8"><option value="recientes">Recientes</option><option value="modelo-az">Modelo A→Z</option><option value="modelo-za">Modelo Z→A</option><option value="nuevos">Nuevos primero</option><option value="semis">Seminuevos primero</option><option value="mezclado">Mezclado</option></Select></div>{sortedUnits.map(unit => <article key={unit.id} className={`flex flex-col gap-3 rounded-xl border border-ink-600 p-3 sm:flex-row sm:items-center sm:justify-between ${rowTone(unit)}`}><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><b className="truncate text-sm">{unit.product?.name}</b><Badge color={badgeTone(unit)}>{statusLabel[unit.status] || unit.status}</Badge></div><p className="mt-1 text-xs text-mute">IMEI {unit.serial} · últimos 4: <b className="text-fore">{unit.serial?.slice(-4)}</b>{unit.location?.name ? ` · ${unit.location.name}` : ''}{unit.batteryHealth ? ` · batería ${unit.batteryHealth}%` : ''}{unit.supplierName ? ` · prov. ${unit.supplierName}` : ''}{unit.originalCost ? ` · costo ${formatCost(unit)}` : ''}</p>{unit.reservationCustomer && <p className="mt-1 text-xs font-semibold text-[#a78bfa]">Atajado por {unit.reservationCustomer} · vence {unit.reservedUntil ? new Date(unit.reservedUntil).toLocaleDateString('es-PY') : '—'}</p>}{unit.notes && <p className="mt-1 text-xs text-fore/80">Nota: {unit.notes}</p>}<p className="mt-1 text-[11px] text-mute">{verifiedLabel(unit) || (unit.lastVerifiedAt ? `Verificado ${new Date(unit.lastVerifiedAt).toLocaleString('es-PY')}` : 'Sin verificación física')}</p></div><div className="flex shrink-0 flex-wrap gap-2">{unit.status === 'AVAILABLE' && <UnitAction label="Vender" icon="money" tone="ok" disabled={busy} onClick={() => sellUnit(unit)} />}{unit.status === 'AVAILABLE' && <UnitAction label="Reservar" icon="calendar" tone="warn" disabled={busy} onClick={() => openReserveFor(unit)} />}{unit.status === 'IN_TRANSIT' && <UnitAction label="Recibir en sucursal" icon="truck" tone="ok" disabled={busy} onClick={() => setArrivalUnit(unit)} />}{unit.status === 'RESERVED' && <UnitAction label="Liberar" icon="unlock" tone="warn" disabled={busy} onClick={() => releaseReservation(unit.serial)} />}{unit.status !== 'SOLD' && <UnitAction label="Verificado" icon="check" tone="ok" disabled={busy} onClick={() => verify(unit)} />}<UnitAction label="Etiqueta" icon="tag" tone="fono" onClick={() => printLabel(unit)} /><UnitAction label="Historial" icon="clock" tone="mute" disabled={busy} onClick={() => openHistory(unit)} /><UnitAction label={unit.status === 'DEFECTIVE' ? 'Habilitar' : 'Ajustar'} icon="settings" tone="warn" disabled={busy || ['SOLD', 'RESERVED', 'IN_TRANSIT'].includes(unit.status)} onClick={() => requestReason('adjust', unit)} /><UnitAction label="Retirar" icon="trash" tone="bad" disabled={busy || unit.status !== 'AVAILABLE'} onClick={() => requestReason('remove', unit)} /></div></article>)}{!units.length && <EmptyState compact icon="box" title="No hay unidades para este filtro." />}</div>}
