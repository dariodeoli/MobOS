...
import { CONDICION_UNIDAD, etiquetaCondicionUnidad, estadoInventario, tonoInventario, nombreProducto, puntoCondicionUnidad, sigueEnInventario, sinCostoUnitario, costoEnGs } from '@/utils/inventario'
import { serialEnmascarado, ultimos4 } from '@/utils/serial'
import { etiquetaDespacho, etiquetaEta } from '@/lib/traslados'
...
const GRID_TRASLADOS = 'grid min-w-[72rem] grid-cols-[minmax(9rem,1.3fr)_minmax(7rem,1.2fr)_4rem_6rem_5.5rem_6rem_6rem_minmax(7rem,0.9fr)_7rem_13rem] items-center gap-x-2'
...
// columna en todos los estados. Cada columna lleva su ícono (donde aplica) y el
// título completo; el detalle largo queda en el tooltip.
function EncabezadoUnidades({ seleccionado = false, onSeleccionar }) {
  const celda = 'flex min-w-0 items-center gap-1 truncate text-[10px] font-bold uppercase tracking-wider text-mute'
  const icono = 'h-3 w-3 shrink-0 opacity-70'
  return (
    <div data-testid="inventario-encabezado" className={`${UNIDADES_GRID} rounded-lg border border-transparent border-b-ink-600/60 border-l-4 bg-ink-800/40 px-3 py-1`}>
...
// Esqueleto de carga con la misma grilla que el encabezado y las filas: mientras
// llegan los datos se ve el esqueleto alineado en lugar del estado vacío
// (antes la tabla "saltaba" del vacío a las filas).
function FilasCargando({ cantidad = 4 }) {
  return (
    <div className="space-y-1" data-testid="unidades-cargando" aria-hidden="true">
      {Array.from({ length: cantidad }).map((_, indice) => (
        <div key={indice} className={`${UNIDADES_GRID} rounded-lg border border-ink-600 border-l-4 border-l-transparent px-3 py-1.5`}>
          <span className="flex items-center"><span className="flex h-11 w-11 items-center justify-center md:h-5 md:w-5"><Skeleton className="h-4 w-4 rounded" /></span></span>
          <span className="flex min-w-0 items-center gap-1.5"><Skeleton className="h-4 w-4 rounded-md" /><Skeleton className="h-3 w-3/5" /></span>
          <span className="flex items-center"><Skeleton className="h-3 w-8" /></span>
          <span className="flex items-center justify-end"><Skeleton className="h-3 w-14" /></span>
          <span className="flex items-center"><Skeleton className="h-3 w-12" /></span>
          <span className="flex items-center"><Skeleton className="h-3 w-24" /></span>
          <span className="flex items-center"><Skeleton className="h-3 w-16" /></span>
          <span className="flex items-center justify-end"><Skeleton className="h-3 w-20" /></span>
        </div>
      ))}
    </div>
  )
...
function MenuAcciones({ unit, busy, acciones }) {
  ...
  return <span className="relative" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}>
    <button type="button" disabled={busy} aria-label={`Acciones de ${unit.serial}`} title="Más acciones" onClick={() => setAbierto((valor) => !valor)} className="grid h-11 w-11 md:h-7 md:w-7 place-items-center rounded-lg border border-ink-600 text-mute transition hover:border-fono/40 hover:text-fore">
...
export default function Inventario({ tab: tabProp, onTabChange } = {}) {
  ...
  function exportarUnidadesSeleccionadas() {
    ...
    const filas = [['Producto', 'IMEI / Serial', 'Condición', 'Ubicación', 'Verificado'], ...lista.map((unit) => [nombreProducto(unit.product || {}), unit.serial || '', etiquetaCondicionUnidad(unit) || unit.condition || '', unit.location?.name || '', unit.lastVerifiedAt ? fechaVerificacion(unit.lastVerifiedAt) : 'Sin verificar'])]
  ...
  const [informeDe, setInformeDe] = useState(null)
  const [certificadoDe, setCertificadoDe] = useState(null)
  ...
  // #218: ETA del lote editable desde la tabla mientras siga en tránsito.
  const [etaPara, setEtaPara] = useState(null)
  ...
  const [transfer, setTransfer] = useState({ sourceBranchId: '', destinationBranchId: '', destinationLocationId: '', productId: '', serials: '', notes: '', eta: '' })
  ...
  // `genero` concuerda el participio con el nombre del documento (Etiqueta →
  // «enviada», Informe → «enviado»).
  const avisarImpresion = (resultado, nombre = 'Etiqueta', genero = 'f') => {
    if (!resultado) return
    const sufijo = genero === 'm' ? 'o' : 'a'
    if (resultado.ok && !resultado.encolado) { setNotice(`${nombre} enviad${sufijo} a la impresora.`); return }
    if (resultado.ok && resultado.encolado) {
      setNotice(resultado.remoto ? `${nombre} encolad${sufijo}: la imprime el puente cuando la reclame.` : `${nombre} encolad${sufijo}: la impresora no respondió y se reintenta sola.`)
      return
    }
  ...
  // #218: la ETA del lote se puede ajustar (o quitar) mientras esté en tránsito.
  async function guardarEtaLote(event) {
    event.preventDefault()
    if (!etaPara?.id) return
    await setAndRefresh(async () => { await resources.transfers.update({ id: etaPara.id, eta: etaPara.eta ? `${etaPara.eta}T12:00:00.000Z` : null }); setEtaPara(null) }, etaPara.eta ? 'ETA del lote actualizada.' : 'ETA del lote quitada.')
  }
  ...
  return <div className={cn('space-y-4', temaV2Activo() && 'tema-v2')}><Card className="p-4 md:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><p className="text-sm text-mute">Cada IMEI es una unidad física con sucursal, ubicación, estado y auditoría.</p><div className="flex shrink-0 flex-wrap items-center gap-2"><Button onClick={abrirReceive}>+ Recibir unidad</Button><Button variant="outline" onClick={() => setReserveOpen(true)}>Reservar</Button><Button variant="outline" onClick={() => setTransferOpen(true)}>Transferir</Button></div></div><form onSubmit={search} className="mt-4 flex flex-wrap gap-2"><SearchField value={query} onChange={event => setQuery(event.target.value)} placeholder="Escanear IMEI, SKU o buscar modelo" ariaLabel="Buscar en inventario" className="min-w-0 flex-1" /><Select value={orden} onChange={event => recordarOrden(event.target.value)} className="w-auto" aria-label="Orden del inventario" title="Se recuerda tu último orden"><option value="recientes">Recientes</option><option value="modelo-az">Modelo A→Z</option><option value="modelo-za">Modelo Z→A</option><option value="nuevos">Nuevos primero</option><option value="semis">Seminuevos primero</option><option value="modelo-natural">Modelo (17→13)</option><option value="mezclado">Modelos mezclados</option><option value="costo-mayor">Costo mayor</option><option value="costo-menor">Costo menor</option></Select><Button type="button" variant="outline" onClick={() => setScannerOpen(true)}>Escanear</Button><Button type="button" variant="outline" onClick={startCount}>Conteo rápido</Button><ListGridToggle value={vistaUnidades} onChange={cambiarVistaUnidades} />{disponibles.length > 0 && <Button type="button" variant="outline" onClick={() => printLabels(disponibles).then(avisarImpresion)}>Etiquetas ({disponibles.length})</Button>}<Button type="button" variant="outline" onClick={() => setGondolaOpen(true)}>Etiquetas de góndola</Button>{tab === 'unidades' && <Button type="button" variant="outline" className="h-9 px-3 text-xs" disabled={exportando || busy} onClick={exportarUnidades}><Icon name="download" className="h-4 w-4" />Exportar CSV</Button>}</form><div className="mt-4 flex gap-1 overflow-x-auto rounded-lg border border-ink-600 bg-ink-800 p-1">{[['unidades', `Inventario (${disponibles.length})`], ['taller', 'Taller'], ...(canViewAlerts ? [['alertas', `Alertas (${(stockAlerts.alerts?.length || 0) + (stockAlerts.outOfStock?.length || 0)})`]] : []), ['reservas', `Reservas (${reservations.length})`], ['traslados', `Traslados (${transfers.length})`], ['vendidos', `Vendidos (${vendidosFiltrados.length})`], ['transito', `En tránsito (${enTransito.length})`], ['ubicaciones', `Ubicaciones (${locations.length})`], ['compartido', 'Compartido'], ['eliminados', `Eliminados (${removedUnits.length})`], ['conteos', 'Conteos']].map(([key, label]) => <button key={key} onClick={() => cambiarTab(key)} className={`min-h-11 shrink-0 rounded-md px-3 py-2 text-xs font-semibold md:min-h-0 ${tab === key ? 'bg-fono/15 text-fono-light' : 'text-mute hover:text-fore'}`}>{label}</button>)}</div>{notice && <Aviso tono="ok" className="mt-3">{notice}</Aviso>}{error && <Aviso tono="error" className="mt-3">{error}</Aviso>}
    <BarraLote cantidad={seleccionados.length} onLimpiar={() => setSeleccionados([])}>
      ...
      <button type="button" disabled={busy} title="Apartar todas las seleccionadas para un cliente" onClick={() => { setReserve(data => ({ ...data, serials: unidadesElegidas().map(unidad => unidad.serial).join(', ') })); setReserveOpen(true) }} className="min-h-11 rounded-lg border border-ink-500 px-2 py-1 text-xs md:min-h-0 font-semibold transition hover:text-fore disabled:opacity-50">Reservar todos</button>
      <button type="button" disabled={busy} title="Marcar en revisión con un motivo (queda auditado)" onClick={() => requestReason('adjust', unidadesElegidas())} className="min-h-11 rounded-lg border border-warn/40 px-2 py-1 text-xs md:min-h-0 font-semibold text-warn transition hover:bg-warn/10 disabled:opacity-50">Enviar a revisión</button>
      <button type="button" disabled={busy} title="Dar de baja con un motivo (quedan en Eliminados)" onClick={() => requestReason('remove', unidadesElegidas())} className="min-h-11 rounded-lg border border-bad/40 px-2 py-1 text-xs md:min-h-0 font-semibold text-bad transition hover:bg-bad/10 disabled:opacity-50">Dar de baja</button>
      <button type="button" className="min-h-11 rounded-lg border border-ink-500 px-2 py-1 text-xs font-semibold transition hover:text-fore md:min-h-0" onClick={copiarImeis}>Copiar IMEIs</button>
      <button type="button" className="min-h-11 rounded-lg border border-ink-500 px-2 py-1 text-xs font-semibold transition hover:text-fore md:min-h-0" onClick={() => printLabels(unidadesElegidas()).then(avisarImpresion)}>Imprimir etiquetas</button>
      <button type="button" className="min-h-11 rounded-lg border border-ink-500 px-2 py-1 text-xs font-semibold transition hover:text-fore md:min-h-0" onClick={exportarUnidadesSeleccionadas}>Exportar CSV</button>
    </BarraLote>
    {tab === 'taller' && (
      <div className="mt-4">
        <TallerRack
...
<EtiquetasProductoModal open={gondolaOpen} onClose={() => setGondolaOpen(false)} productos={products} />
    <DocumentoUnidadModal unit={informeDe} tipo="informe" open={Boolean(informeDe)} onClose={() => setInformeDe(null)} onResult={(resultado) => avisarImpresion(resultado, 'Informe', 'm')} />
    <DocumentoUnidadModal unit={certificadoDe} tipo="certificado" open={Boolean(certificadoDe)} onClose={() => setCertificadoDe(null)} onResult={(resultado) => avisarImpresion(resultado, 'Certificado', 'm')} />
    <DocumentoUnidadModal unit={constanciaDe} tipo="constancia" open={Boolean(constanciaDe)} onClose={() => setConstanciaDe(null)} onResult={(resultado) => avisarImpresion(resultado, 'Constancia', 'f')} /></div>}
