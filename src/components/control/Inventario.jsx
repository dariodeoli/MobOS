...
import { Card, Button, Input, Badge, Modal, Select, Textarea, EmptyState, Skeleton, MoneyInput, Dot, Label, ConfirmDialog, useToast } from '@/components/ui'
...
import { leerEtiqueta, qrUnidad } from '@/lib/printing/qr'
...
// Impresión masiva: una página de 58 mm por unidad (por lote o ubicación).
async function printLabels(units = []) {
  ...
  for (const unit of units) {
    ...
    const lastFour = safe(ultimos4(unit.serial) || '----')
    const code = `MOBOS:${unit.serial}`
    const barcodeSvg = document.createElementNS('http://www.w3.org/2000/svg', 'svg')
    JsBarcode(barcodeSvg, code, { format: 'CODE128', displayValue: false, width: 1.4, height: 34, margin: 0 })
    let qr = ''
    const enlace = qrUnidad(unit.serial)
    try { if (enlace) qr = await QRCode.toDataURL(enlace, { errorCorrectionLevel: 'M', margin: 0, width: 160 }) } catch { /* conserva el código de barras */ }
  ...
  const resultado = await imprimirDocumento(ticketEtiquetasUnidad(units), { tipo: 'etiquetas-stock' })
...
async function printLabel(unit) {
  ...
  let qr = ''
  const enlace = qrUnidad(unit.serial)
  try { if (enlace) qr = await QRCode.toDataURL(enlace, { errorCorrectionLevel: 'M', margin: 0, width: 160 }) } catch { /* La etiqueta conserva el código de barras si el QR no puede renderizarse. */ }
  ...
  const resultado = await imprimirDocumento(ticketEtiquetaUnidad(unit), { tipo: 'etiqueta-stock' })
  // Diálogo solo si el fallo fue CLARO (nada se envió ni quedó en cola):
  // tras un resultado incierto o encolado abrirlo podría duplicar el ticket.
  if (puedeCaerAlDialogo(resultado)) imprimirConDialogo(html)
  return resultado
...
export default function Inventario({ tab: tabProp, onTabChange } = {}) {
  ...
  const [vistaUnidades, setVistaUnidades] = useState(() => localStorage.getItem('mobos:inventario-vista') || 'list')
  const [vistaUbicaciones, setVistaUbicaciones] = useState(() => localStorage.getItem('mobos:ubicaciones-vista') || 'grid')
  ...
  async function receiveUnit(event) { event.preventDefault(); const lineas = receive.serial.split(/[\n,;]+/).map(normalizeScan).filter(Boolean); const ubicacion = lineas.find((linea) => linea.startsWith('UBI:')); const ubicacionValida = ubicacion && locations.some((location) => location.id === ubicacion.slice(4) && location.branchId === receive.branchId); if (ubicacionValida) setReceive((data) => ({ ...data, locationId: ubicacion.slice(4) })); const serials = lineas.filter((linea) => !linea.startsWith('UBI:')); if (!serials.length) { setError('Indicá al menos un IMEI/serial.'); return } const locationId = ubicacionValida ? ubicacion.slice(4) : receive.locationId
    ...
    await setAndRefresh(async () => {
      ...
      setReceive({ productId: '', serial: '', branchId: '', locationId: '', condition: 'NEW', batteryHealth: '', supplierName: '', costCurrency: 'PYG', originalCost: '', exchangeRatePyg: '', notes: '' }); setReceiveOpen(false)
  ...
  async function receiveArrival(event) {
    ...
    await setAndRefresh(async () => { await resources.inventoryUnits.verify({ serial: unit.serial, ...(locationId ? { locationId } : {}) }); setArrivalUnit(null); setArrivalLocationId('') }, `IMEI ${ultimos4(unit.serial)} recibido en sucursal y disponible en stock.`)
  }
  ...
  async function copiarRemitoQr() {
    ...
    if (!url) return
    try { await navigator.clipboard.writeText(url); setNotice('Enlace del remito copiado al portapapeles.') } catch { setRemitoQrError('No se pudo copiar el enlace.') }
  }
