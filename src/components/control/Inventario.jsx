...
export default function Inventario({ tab: tabProp, onTabChange } = {}) {
  ...
  const [tabInterno, setTabInterno] = useState('unidades'), [query, setQuery] = useState(''), [busy, setBusy] = useState(false), [error, setError] = useState(''), [notice, setNotice] = useState(''), [orden, setOrden] = useState('recientes')
  ...
  const tabValida = Boolean(tabProp) && INVENTARIO_TABS.includes(tabProp) && (tabProp !== 'alertas' || canViewAlerts)
  const tab = tabValida ? tabProp : tabInterno
  useEffect(() => {
    if (tabProp && !tabValida) onTabChange?.('unidades')
  }, [tabProp, tabValida, onTabChange])
  function cambiarTab(next) { setTabInterno(next); onTabChange?.(next) }
