...
const verifiedLabel = (unit, perfil) => {
...
const UNIDADES_GRID = 'grid min-w-[70rem] grid-cols-[minmax(0,1.5fr)_10.5rem_9.5rem_8.5rem_4.5rem_7.5rem_7rem_8rem_8.5rem] items-center gap-x-3'

function EncabezadoUnidades() {
  const celda = 'text-[10px] font-bold uppercase tracking-wider text-mute'
  return (
    <div className={`${UNIDADES_GRID} px-3 pb-1`}>
      ...
      <span className={celda}>Batería</span>
...
function FilaUnidad({ unit, perfilEmpresa, onClick, onVerify, busy }) {
  ...
  return <div role="button" tabIndex={0} onClick={onClick} onKeyDown={event => { if (event.key === 'Enter') onClick() }} className={`${UNIDADES_GRID} cursor-pointer rounded-lg border border-ink-600 px-3 py-2 transition hover:border-fono/40 ${rowTone(unit)}`}>
    <span className="flex min-w-0 items-center gap-2">
      <b className="truncate text-[13px] leading-snug">{unit.product?.name}</b>
    ...
    <span className="flex min-w-0 items-center gap-2 text-[11px] text-mute">
      <span className="grid h-6 w-6 shrink-0 place-items-center rounded-full bg-ink-700 text-[9px] font-bold text-fore">{iniciales}</span>
      <span className="truncate" title={v?.quien || undefined}>{v ? `${v.quien || unit.verifiedByCode || '—'} · ${fechaVerificacion(unit.lastVerifiedAt)}` : 'Sin verificar'}</span>
    </span>
    <span className="truncate font-mono text-[11px] text-mute" title={serial || undefined}>{serial ? <>{serial.slice(0, -4)}<b className="text-fore">{serial.slice(-4)}</b></> : '—'}</span>
    ...
    <span className="min-w-0">
      <Badge color={badgeTone(unit)}>{statusLabel[unit.status] || unit.status}</Badge>
    ...
    <span className="flex items-center justify-end gap-1.5">
      <button type="button" disabled={busy} title="Registrar verificación física ahora" onClick={event => { event.stopPropagation(); onVerify?.(unit) }} className="rounded-lg border border-ok/40 px-2 py-1 text-[10px] font-bold text-ok transition hover:bg-ok/10 disabled:opacity-50">✓ Verificar</button>
      <Icon name="chevron" className="h-3.5 w-3.5 shrink-0 -rotate-90 text-mute transition" />
    </span>
  </div>
}

function TarjetaUnidad({ unit, perfilEmpresa, onClick }) {
  const v = verifiedLabel(unit, perfilEmpresa)
  const serial = String(unit.serial || '')
  return <button type="button" onClick={onClick} className={`group flex w-full flex-col rounded-2xl border border-ink-600 p-3 text-left transition hover:border-fono/40 ${rowTone(unit)}`}>
    <span className="flex items-start justify-between gap-2">
      <b className="min-w-0 truncate text-[13px]">{unit.product?.name}</b>
      <Badge color={badgeTone(unit)}>{statusLabel[unit.status] || unit.status}</Badge>
    </span>
    <span className="mt-1 block truncate font-mono text-[11px] text-mute">{serial.slice(0, -4)}<b className="text-fore">{serial.slice(-4)}</b></span>
...
async function printLabels(units = []) {
  ...
  for (const unit of units) {
    ...
    let qr = ''
    try { qr = await QRCode.toDataURL(code, { errorCorrectionLevel: 'M', margin: 0, width: 160 }) } catch { /* conserva el código de barras */ }
...
export default function Inventario() {
  ...
  const [tab, setTab] = useState('unidades'), [query, setQuery] = useState(''), [busy, setBusy] = useState(false), [error, setError] = useState(''), [notice, setNotice] = useState(''), [orden, setOrden] = useState('recientes')
  ...
  const disponibles = ordenarUnidades(filtroBusqueda(units.filter(unit => unit.status === 'AVAILABLE')))
  ...
  function openReserveFor(unit) {
    ...
    setReserve({ serials: unit.serial, customerName: '', hours: '2' })
    setReserveMatches([])
    setReserveOpen(true)
  }
  ...
  return <div className="space-y-4"><Card className="p-4 md:p-5"><PageHeader title="Inventario operativo" subtitle="Cada IMEI es una unidad física con sucursal, ubicación, estado y auditoría." actions={<><Button onClick={() => setReceiveOpen(true)}>+ Recibir unidad</Button><Button variant="outline" onClick={() => setReserveOpen(true)}>Reservar</Button><Button variant="outline" onClick={() => setTransferOpen(true)}>Transferir</Button></>} /><form onSubmit={search} className="mt-4 flex flex-wrap gap-2"><Input value={query} onChange={event => setQuery(event.target.value)} placeholder="Escanear IMEI, SKU o buscar modelo" autoCapitalize="characters" className="min-w-0 flex-1" /><Select value={orden} onChange={event => setOrden(event.target.value)} className="w-auto"><option value="recientes">Recientes</option><option value="modelo-az">Modelo A→Z</option><option value="modelo-za">Modelo Z→A</option><option value="nuevos">Nuevos primero</option><option value="semis">Seminuevos primero</option><option value="mezclado">Modelos mezclados</option></Select><Button type="button" variant="outline" onClick={() => setScannerOpen(true)}>Escanear</Button><Button type="button" variant="outline" onClick={startCount}>Conteo rápido</Button><ListGridToggle value={vistaUnidades} onChange={(next) => { setVistaUnidades(next); localStorage.setItem('mobos:inventario-vista', next) }} />{disponibles.length > 0 && <Button type="button" variant="outline" onClick={() => printLabels(disponibles)}>Etiquetas ({disponibles.length})</Button>}<Button type="submit" variant="outline" disabled={busy}>Buscar</Button></form><div className="mt-4 flex gap-1 overflow-x-auto rounded-lg border border-ink-600 bg-ink-800 p-1">{[['unidades', `Unidades (${disponibles.length})`], ...(canViewAlerts ? [['alertas', `Alertas (${(stockAlerts.alerts?.length || 0) + (stockAlerts.outOfStock?.length || 0)})`]] : []), ['reservas', `Reservas (${reservations.length})`], ['traslados', `Traslados (${transfers.length})`], ['vendidos', `Vendidos (${vendidos.length})`], ['transito', `En tránsito (${enTransito.length})`], ['ubicaciones', `Ubicaciones (${locations.length})`], ['compartido', 'Compartido'], ['eliminados', `Eliminados (${removedUnits.length})`]].map(([key, label]) => <button key={key} onClick={() => setTab(key)} className={`shrink-0 rounded-md px-3 py-2 text-xs font-semibold ${tab === key ? 'bg-fono/15 text-fono-light' : 'text-mute hover:text-fore'}`}>{label}</button>)}</div>{notice && <p className="mt-3 rounded-lg border border-ok/30 bg-ok/10 px-3 py-2 text-sm text-ok">{notice}</p>}{error && <p className="mt-3 rounded-lg border border-bad/30 bg-bad/10 px-3 py-2 text-sm text-bad">{error}</p>}
    ...
    {tab === 'vendidos' && <div className="mt-4 space-y-1">{vendidos.map(unit => <FilaUnidad key={unit.id} unit={unit} perfilEmpresa={perfilEmpresa} onClick={() => setDetalleUnidad(unit)} />)}{!vendidos.length && <EmptyState compact icon="box" title={query ? 'Ninguna unidad coincide con la búsqueda.' : 'Todavía no hay vendidos en el período.'} />}</div>}
    {tab === 'transito' && <div className="mt-4 space-y-1">{enTransito.map(unit => <FilaUnidad key={unit.id} unit={unit} perfilEmpresa={perfilEmpresa} onClick={() => setDetalleUnidad(unit)} />)}{!enTransito.length && <EmptyState compact icon="box" title={query ? 'Ninguna unidad coincide con la búsqueda.' : 'No hay unidades en tránsito.'} />}</div>}
{tab === 'alertas' && <div className="mt-4 space-y-4">{alertsLoading && <div className="space-y-2"><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /></div>}{alertsError && <p className="rounded-lg border border-bad/30 bg-bad/10 px-3 py-2 text-sm text-bad">{alertsError}</p>}{!alertsLoading && !alertsError && !(stockAlerts.alerts?.length || stockAlerts.outOfStock?.length) && <EmptyState compact icon="check" title="Sin alertas de reposición." description="Todo el stock está por encima de su umbral." />}{!alertsLoading && stockAlerts.alerts?.length > 0 && <section><h3 className="text-xs font-bold uppercase tracking-wider text-mute">Bajo el umbral de reposición</h3><div className="mt-2 space-y-2">{stockAlerts.alerts.map(item => <article key={item.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-warn/25 bg-warn/5 px-3 py-2"><div className="min-w-0"><b className="text-sm">{item.name}</b><p className="mt-1 text-xs text-mute">{item.sku ? `SKU ${item.sku} · ` : ''}Stock {item.stock} de {item.reorderPoint}{item.branchName ? ` · ${item.branchName}` : ''}</p></div><div className="flex shrink-0 items-center gap-2"><Badge color="orange">Reponer</Badge><Button type="button" variant="outline" disabled={busy} onClick={() => { setThreshold({ id: item.id, name: item.name }); setThresholdValue(String(item.reorderPoint ?? '')) }}>Ajustar umbral</Button></div></article>)}</div></section>}{!alertsLoading && stockAlerts.outOfStock?.length > 0 && <section><h3 className="text-xs font-bold uppercase tracking-wider text-mute">Agotados</h3><div className="mt-2 space-y-2">{stockAlerts.outOfStock.map(item => <article key={item.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-bad/25 bg-bad/5 px-3 py-2"><div className="min-w-0"><b className="text-sm">{item.name}</b><p className="mt-1 text-xs text-mute">{item.sku ? `SKU ${item.sku} · ` : ''}Sin stock{item.branchName ? ` · ${item.branchName}` : ''}</p></div><div className="flex shrink-0 items-center gap-2"><Badge color="red">Agotado</Badge><Button type="button" variant="outline" disabled={busy} onClick={() => { setThreshold({ id: item.id, name: item.name }); setThresholdValue(String(item.reorderPoint ?? '')) }}>Definir umbral</Button></div></article>)}</div></section>}</div>}
    {tab === 'reservas' && <div className="mt-4 space-y-1.5">{reservations.map(unit => <article key={unit.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-[#8b5cf6]/30 bg-[#8b5cf6]/10 px-3 py-2"><span className="min-w-0 flex-1"><b className="block truncate text-[13px]">{unit.product?.name}</b><span className="mt-0.5 block truncate text-[11px] text-mute">IMEI <span>{String(unit.serial).slice(0, -4)}<b className="text-fore">{String(unit.serial).slice(-4)}</b></span>{unit.reservationCustomer ? ` · Atajado por ${unit.reservationCustomer}` : ''}{unit.reservedUntil ? ` · vence ${new Date(unit.reservedUntil).toLocaleString('es-PY', { dateStyle: 'short', timeStyle: 'short' })}` : ''}</span></span><span className="flex shrink-0 gap-1.5"><Button type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => printReservationReceipt(unit, { format: 'a4' })}>Comprobante</Button><Button variant="outline" className="h-8 px-2 text-xs" disabled={busy} onClick={() => releaseReservation(unit.serial)}>Liberar</Button></span></article>)}{!reservations.length && <EmptyState compact icon="box" title="No hay reservas activas." />}</div>}
    {tab === 'traslados' && <div className="mt-4 space-y-1.5">{transfers.map(item => <article key={item.id} className="rounded-lg border border-ink-600 px-3 py-2"><div className="flex flex-wrap items-center justify-between gap-2"><span className="flex min-w-0 items-center gap-2"><b className="truncate text-[13px]">{item.sourceBranch?.name} → {item.destinationBranch?.name}</b>{item.aexGuide ? <Badge color="blue">AEX {item.aexGuide}</Badge> : null}</span><span className="text-[11px] text-mute">{new Date(item.createdAt).toLocaleString('es-PY', { dateStyle: 'short', timeStyle: 'short' })}</span></div><p className="mt-1 text-xs text-mute">{item.lines?.map(line => `${line.quantity} × ${line.sourceProduct?.name}`).join(' · ')}{item.notes ? ` · ${item.notes}` : ''}</p><div className="mt-2 flex flex-wrap items-center gap-2">{item.aexGuide ? <><span className="text-xs font-semibold text-fono-light">Guía AEX {item.aexGuide}</span><Button type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => seguirGuia(item)}>Seguimiento</Button></> : guiaPara?.id === item.id ? <form onSubmit={guardarGuia} className="flex flex-wrap items-center gap-2"><Input className="h-8 w-44" value={guiaPara.guia} onChange={event => setGuiaPara(current => ({ ...current, guia: event.target.value }))} placeholder="Guía AEX (ej: A003526979)" /><Button type="submit" className="h-8 px-2 text-xs" disabled={busy}>Guardar</Button><Button type="button" variant="ghost" className="h-8 px-2 text-xs" onClick={() => setGuiaPara(null)}>Cancelar</Button></form> : <Button type="button" variant="outline" className="h-8 px-2 text-xs" disabled={busy} onClick={() => setGuiaPara({ id: item.id, guia: '' })}>Agregar guía AEX</Button>}</div></article>)}{!transfers.length && <EmptyState compact icon="box" title="Todavía no hay transferencias." />}</div>}
    {tab === 'ubicaciones' && <div className="mt-4 space-y-3">{canManageLocations && <div className="flex flex-wrap gap-2"><Button type="button" onClick={() => setLocationForm({ id: null, name: '', code: '', branchId: '' })}>+ Crear ubicación</Button><ListGridToggle value={vistaUbicaciones} onChange={(next) => { setVistaUbicaciones(next); localStorage.setItem('mobos:ubicaciones-vista', next) }} /></div>}<div className={vistaUbicaciones === 'grid' ? 'grid gap-2 sm:grid-cols-2' : 'space-y-1.5'}>{locations.map(location => { const sample = locationSample(units, location.id); return <article key={location.id} className="rounded-xl border border-ink-600 p-3"><div className="flex justify-between gap-2"><b className="text-sm">{location.name}</b><Badge color={location.isActive ? 'green' : 'slate'}>{location.isActive ? 'Activa' : 'Inactiva'}</Badge></div><p className="mt-1 text-xs text-mute">{location.branch?.name} · {location._count?.inventoryUnits || 0} unidades{location.code ? ` · ${location.code}` : ''}</p>{sample.length > 0 ? <div className="mt-2 flex flex-wrap gap-1.5">{sample.map(unit => <span key={unit.id} className="inline-flex max-w-full items-center gap-1.5 rounded-lg border border-ink-500 bg-ink-800 px-2 py-0.5"><Dot color={badgeTone(unit)} className="h-1.5 w-1.5" /><span className="max-w-[9rem] truncate text-[11px] text-fore">{unit.product?.name}</span></span>)}</div> : <p className="mt-2 text-[11px] text-mute">Sin unidades</p>}{canManageLocations && <div className="mt-2 flex gap-3"><button type="button" className="text-xs font-semibold text-fono-light hover:underline" onClick={() => setLocationForm({ id: location.id, name: location.name, code: location.code || '', branchId: location.branchId })}>Editar</button><button type="button" className="text-xs font-semibold text-fono-light hover:underline" onClick={() => printLocationLabel(location)}>Etiqueta</button><button type="button" className="text-xs font-semibold text-fono-light hover:underline" onClick={() => printLabels(units.filter(unit => unit.locationId === location.id))}>Etiquetas</button><button type="button" className="text-xs font-semibold text-mute hover:underline" disabled={busy} onClick={() => toggleLocation(location)}>{location.isActive ? 'Desactivar' : 'Reactivar'}</button></div>}</article> })}{!locations.length && <EmptyState compact icon="box" title={canManageLocations ? 'Creá tu primera ubicación para organizar el stock.' : 'Todavía no hay ubicaciones cargadas.'} />}</div></div>}
    ...
    {tab === 'eliminados' && <div className="mt-4 space-y-1.5">{removedUnits.map(unit => <article key={unit.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-bad/25 bg-bad/5 px-3 py-2"><span className="min-w-0 flex-1"><b className="block truncate text-[13px]">{unit.product?.name}</b><span className="mt-0.5 block truncate text-[11px] text-mute">IMEI <span>{unit.serial?.slice(0, -4)}<b className="text-fore">{unit.serial?.slice(-4)}</b></span> · conserva su historial de auditoría</span></span><Button className="h-8 shrink-0 px-2 text-xs" disabled={busy} onClick={() => requestReason('restore', unit)}>Restaurar</Button></article>)}{!removedUnits.length && <EmptyState compact icon="box" title="No hay unidades eliminadas recuperables." />}</div>}
