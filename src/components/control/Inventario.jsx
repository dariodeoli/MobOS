...
import { printReservationReceipt, printTransferReceipt, transferReceiveUrlFor } from '@/components/shared/OrderReceipt'
...
export default function Inventario({ tab: tabProp, onTabChange } = {}) {
  ...
  const avisarImpresion = (resultado) => {
    if (!resultado) return
    if (resultado.ok && !resultado.encolado) { setNotice('Etiqueta enviada a la impresora.'); return }
    if (resultado.ok && resultado.encolado) {
      setNotice(resultado.remoto ? 'Etiqueta encolada: la imprime el puente cuando la reclame.' : 'Etiqueta encolada: la impresora no respondió y se reintenta sola.')
      return
    }
    if (!puedeCaerAlDialogo(resultado)) setError(resultado.error || 'No se pudo imprimir la etiqueta.')
  }
