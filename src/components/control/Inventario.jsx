...
import { ticketEtiquetaUbicacion, ticketEtiquetaUnidad, ticketEtiquetasUnidad, ticketRemito, ticketReserva } from '@/lib/printing/tickets'
...
const GRID_TRASLADOS = 'grid min-w-[50rem] grid-cols-[minmax(9rem,1.4fr)_minmax(9rem,1.6fr)_6rem_7rem_11rem] items-center gap-x-2'
...
function FilaUnidad({ unit, perfilEmpresa, onClick, onVerify, onSell, busy }) {
  ...
  return <div role="button" tabIndex={0} data-testid="inventario-fila" onClick={onClick} onKeyDown={event => { if (event.key === 'Enter') onClick() }} className={`${UNIDADES_GRID} cursor-pointer rounded-lg border border-ink-600 px-3 py-2 transition hover:border-fono/40 ${rowTone(unit)}`}>
    ...
    <span className="flex min-w-0 items-center gap-1.5 text-[11px] text-mute">
      <span className="grid h-5 w-5 shrink-0 place-items-center rounded-full bg-ink-700 text-[8px] font-bold text-fore">{iniciales}</span>
      <span className="truncate" title={v ? `Verificó ${v.quien || unit.verifiedByCode || '—'}` : undefined}>{v ? fechaVerificacion(unit.lastVerifiedAt) : 'Sin verificar'}</span>
    </span>
...
async function imprimirReserva(unit) {
  const resultado = await imprimirTicketOFallback(ticketReserva(unit), '')
  if (!resultado.directo) await printReservationReceipt(unit, { format: 'a4' })
}

async function imprimirRemito(transfer) {
  const resultado = await imprimirTicketOFallback(ticketRemito(transfer), '')
  if (!resultado.directo) await printTransferReceipt(transfer, { format: 'a4' })
}
...
{tab === 'alertas' && <div className="mt-4 space-y-4">{alertsLoading && <div className="space-y-2"><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /><Skeleton className="h-16 w-full" /></div>}{alertsError && <p className="rounded-lg border border-bad/30 bg-bad/10 px-3 py-2 text-sm text-bad">{alertsError}</p>}{!alertsLoading && !alertsError && !(stockAlerts.alerts?.length || stockAlerts.outOfStock?.length) && <EmptyState compact icon="check" title="Sin alertas de reposición." description="Todo el stock está por encima de su umbral." />}{!alertsLoading && stockAlerts.alerts?.length > 0 && <section><h3 className="text-xs font-bold uppercase tracking-wider text-mute">Bajo el umbral de reposición</h3><div className="mt-2 space-y-2">{stockAlerts.alerts.map(item => <article key={item.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-warn/25 bg-warn/5 px-3 py-2"><div className="min-w-0"><b className="text-sm">{item.name}</b><p className="mt-1 text-xs text-mute">{item.sku ? `SKU ${item.sku} · ` : ''}Stock {item.stock} de {item.reorderPoint}{item.branchName ? ` · ${item.branchName}` : ''}</p></div><div className="flex shrink-0 items-center gap-2"><Badge color="orange">Reponer</Badge><Button type="button" variant="outline" disabled={busy} onClick={() => { setThreshold({ id: item.id, name: item.name }); setThresholdValue(String(item.reorderPoint ?? '')) }}>Ajustar umbral</Button></div></article>)}</div></section>}{!alertsLoading && stockAlerts.outOfStock?.length > 0 && <section><h3 className="text-xs font-bold uppercase tracking-wider text-mute">Agotados</h3><div className="mt-2 space-y-2">{stockAlerts.outOfStock.map(item => <article key={item.id} className="flex flex-wrap items-center gap-2 rounded-lg border border-bad/25 bg-bad/5 px-3 py-2"><div className="min-w-0"><b className="text-sm">{item.name}</b><p className="mt-1 text-xs text-mute">{item.sku ? `SKU ${item.sku} · ` : ''}Sin stock{item.branchName ? ` · ${item.branchName}` : ''}</p></div><div className="flex shrink-0 items-center gap-2"><Badge color="red">Agotado</Badge><Button type="button" variant="outline" disabled={busy} onClick={() => { setThreshold({ id: item.id, name: item.name }); setThresholdValue(String(item.reorderPoint ?? '')) }}>Definir umbral</Button></div></article>)}</div></section>}</div>}
    {tab === 'reservas' && <div className="mt-4 overflow-x-auto" data-testid="reservas-tabla"><div className={cn(GRID_RESERVAS, 'px-3.5 pb-2 pt-1')}><span className={CELDA_INV}>Producto</span><span className={CELDA_INV}>IMEI</span><span className={CELDA_INV}>Cliente</span><span className={CELDA_INV}>Vence</span><span className={cn(CELDA_INV, 'text-right')}>Acciones</span></div><div className="space-y-1">{reservations.map(unit => { const serial = String(unit.serial || ''); const cliente = unit.reservationCustomerRef?.name || unit.reservationCustomer || 'Sin cliente'; return <div key={unit.id} data-testid="reserva-fila" className={cn(GRID_RESERVAS, 'rounded-xl border border-[#8b5cf6]/30 bg-[#8b5cf6]/10 px-3.5 py-2')}><span className="truncate text-[13px] font-semibold" title={nombreProducto(unit.product || {})}>{nombreProducto(unit.product || {}) || 'Producto'}</span><SerialTexto serial={serial} className="truncate text-[11px] text-mute" /><span className="truncate text-xs" title={cliente}>{cliente}</span><span className="truncate text-xs text-mute" title={unit.reservedUntil ? new Date(unit.reservedUntil).toLocaleString('es-PY') : undefined}>{unit.reservedUntil ? fechaReserva(unit.reservedUntil) : '—'}</span><span className="flex flex-wrap items-center justify-end gap-1"><Button type="button" className="h-8 px-2 text-xs" disabled={busy} onClick={() => sellUnit(unit)}>Vender</Button><Button type="button" variant="outline" className="h-8 px-2 text-xs" title="Imprimir comprobante de la reserva" onClick={() => imprimirReserva(unit)}>Comprobante</Button><Button variant="outline" className="h-8 px-2 text-xs" disabled={busy} onClick={() => releaseReservation(unit.serial)}>Liberar</Button></span></div> })}{!reservations.length && <EmptyState compact icon="box" title="No hay reservas activas." />}</div></div>}
    {tab === 'traslados' && <div className="mt-4 overflow-x-auto" data-testid="traslados-tabla"><div className={cn(GRID_TRASLADOS, 'px-3.5 pb-2 pt-1')}><span className={CELDA_INV}>Ruta</span><span className={CELDA_INV}>Líneas</span><span className={CELDA_INV}>Fecha</span><span className={CELDA_INV}>Guía AEX</span><span className={cn(CELDA_INV, 'text-right')}>Acciones</span></div><div className="space-y-1">{transfers.map(item => { const lineas = item.lines?.map(line => `${line.quantity} × ${line.sourceProduct?.name}`).join(' · ') || 'Sin líneas'; return <div key={item.id} data-testid="traslado-fila" className={cn(GRID_TRASLADOS, 'rounded-xl border border-ink-600 bg-ink-800/40 px-3.5 py-2')}><span className="truncate text-[13px] font-semibold" title={`${item.sourceBranch?.name} → ${item.destinationBranch?.name}`}>{item.sourceBranch?.name} → {item.destinationBranch?.name}</span><span className="truncate text-xs text-mute" title={[lineas, item.notes].filter(Boolean).join(' · ')}>{lineas}{item.notes ? <span className="text-mute/70"> · {item.notes}</span> : null}</span><span className="truncate text-xs text-mute">{new Date(item.createdAt).toLocaleDateString('es-PY', { day: '2-digit', month: 'short' }).replace('.', '')}</span><span className="truncate font-mono text-[11px] text-fono-light" title={item.aexGuide || undefined}>{item.aexGuide || '—'}</span><span className="flex flex-wrap items-center justify-end gap-1"><Button type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => imprimirRemito(item)}>Remito</Button>{!item.aexGuide && <Button type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => cotizarEnvio(item)}>Enviar por AEX</Button>}{item.aexGuide ? <Button type="button" variant="outline" className="h-8 px-2 text-xs" onClick={() => seguirGuia(item)}>Seguimiento</Button> : guiaPara?.id === item.id ? <form onSubmit={guardarGuia} className="flex flex-wrap items-center gap-1"><Input className="h-8 w-40" aria-label="Guía AEX" value={guiaPara.guia} onChange={event => setGuiaPara(current => ({ ...current, guia: event.target.value }))} placeholder="A003526979" /><Button type="submit" className="h-8 px-2 text-xs" disabled={busy}>Guardar</Button><Button type="button" variant="ghost" className="h-8 px-2 text-xs" onClick={() => setGuiaPara(null)}>Cancelar</Button></form> : <Button type="button" variant="outline" className="h-8 px-2 text-xs" disabled={busy} onClick={() => setGuiaPara({ id: item.id, guia: '' })}>Agregar guía</Button>}</span></div> })}{!transfers.length && <EmptyState compact icon="box" title="Todavía no hay transferencias." />}</div></div>}
