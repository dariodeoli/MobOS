...
export default function InformePublico() {
  ...
  return <div className="mx-auto max-w-xl space-y-4 p-4 sm:p-6">
    <Card className="p-5">
      ...
      {informe.repuestosNoOem && <div className="mt-3 rounded-lg border border-ink-600 p-3"><p className={ROTULO_SECCION}>Repuestos no-OEM</p><p className="mt-1 text-sm font-semibold text-fore">{informe.repuestosNoOem}</p>{informe.repuestosNoOemNota && <p className="mt-1 text-xs text-mute">{informe.repuestosNoOemNota}</p>}</div>}<Nota tono="warn" className="mt-3">{informe.aviso}</Nota>
    </Card>
