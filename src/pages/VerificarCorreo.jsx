...
export default function VerificarCorreo() {
  ...
  return <main className="flex min-h-dvh flex-col bg-[#071018] text-white"><div className="flex flex-1 items-center justify-center p-5"><Card className="w-full max-w-md border-white/10 bg-[#0b1822]"><img src="/logo-dark.svg" alt="MobOS" className="w-40" /><h1 className="mt-6 text-2xl font-bold">Verificación de correo</h1>{state.loading && <p role="status" className="mt-4 text-sm text-mute">Verificando el enlace…</p>}{state.error && <p role="alert" className="mt-4 rounded-lg border border-bad/30 bg-bad/10 p-3 text-sm text-bad">{state.error}</p>}{state.message && <p role="status" className="mt-4 rounded-lg border border-ok/30 bg-ok/10 p-3 text-sm text-ok">{state.message}</p>}<Link to="/login" className="mt-5 inline-flex min-h-11 items-center text-sm font-semibold text-fono-light hover:text-white">Ir al acceso</Link></Card></div><ProductFooter /></main>
}
