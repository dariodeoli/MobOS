...
export default function RecuperarContrasena() {
  ...
  return <main className="flex min-h-dvh items-center justify-center bg-[#071018] p-5 text-white"><Card className="w-full max-w-md border-white/10 bg-[#0b1822]"><Link to="/login" className="text-sm font-semibold text-fono-light hover:text-white">← Volver al acceso</Link><img src="/logo-dark.svg" alt="MobOS" className="mt-6 w-40" /><h1 className="mt-6 text-2xl font-bold">{resetting ? 'Elegí una nueva contraseña' : 'Recuperá tu contraseña'}</h1><p className="mt-2 text-sm leading-6 text-mute">{resetting ? 'La nueva contraseña cerrará las sesiones activas de tu empresa.' : 'Te enviaremos un enlace de un solo uso si existe una cuenta con ese correo.'}</p><form onSubmit={submit} className="mt-6 space-y-4">{resetting ? <><div><Label htmlFor="new-password">Nueva contraseña</Label><Input id="new-password" type="password" minLength={12} maxLength={72} required autoComplete="new-password" value={password} onChange={event => setPassword(event.target.value)} /></div><div><Label htmlFor="confirm-password">Repetir contraseña</Label><Input id="confirm-password" type="password" minLength={12} maxLength={72} required autoComplete="new-password" value={confirm} onChange={event => setConfirm(event.target.value)} /></div></> : <div><Label htmlFor="recovery-email">Correo de la empresa</Label><Input id="recovery-email" type="email" required autoComplete="email" value={email} onChange={event => setEmail(event.target.value)} placeholder="vos@tutienda.com" /></div>}{error && <p role="alert" className="rounded-lg border border-bad/30 bg-bad/10 p-3 text-sm text-bad">{error}</p>}{message && <p role="status" className="rounded-lg border border-ok/30 bg-ok/10 p-3 text-sm text-ok">{message}</p>}<Button className="w-full" type="submit" disabled={saving}>{saving ? 'Procesando…' : resetting ? 'Actualizar contraseña' : 'Enviar instrucciones'}</Button></form><p className="mt-6 text-center text-xs text-slate-500">{APP_VERSION} · MobOS</p></Card></main>
}
