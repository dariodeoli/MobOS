...
export function getDemoCash() { return read() }
...
export function openDemoCash(openingPyg = DEFAULT.openingPyg, notes = '') {
  const next = { ...DEFAULT, openedAt: new Date().toISOString(), openingPyg: Number(openingPyg) || 0, notes, status: 'OPEN' }
  localStorage.setItem(KEY, JSON.stringify(next))
  return next
}
export function closeDemoCash(countedPyg, expectedPyg, notes = '') {
  const current = read()
  const next = { ...current, countedPyg: Number(countedPyg) || 0, expectedPyg: Number(expectedPyg) || 0, differencePyg: (Number(countedPyg) || 0) - (Number(expectedPyg) || 0), closedAt: new Date().toISOString(), closedById: 'demo-user', status: 'CLOSED', notes }
  localStorage.setItem(KEY, JSON.stringify(next))
  return next
}
