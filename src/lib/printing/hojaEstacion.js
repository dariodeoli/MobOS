...
export function buildStationSheetHtml(unidades = [], { estacion = 'Taller', fecha = new Date() } = {}) {
  ...
  const fechaTexto = fecha instanceof Date && !Number.isNaN(fecha.getTime())
    ? fecha.toLocaleString('es-PY', { day: '2-digit', month: '2-digit', year: 'numeric', hour: '2-digit', minute: '2-digit' })
    : ''

  return `<!doctype html><html lang="es"><head><meta charset="utf-8"><title>Hoja de estación · ${escapeHtml(estacion)}</title><style>
