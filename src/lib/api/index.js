...
export const resources = {
  ...
  inventoryUnits: { list: (q = '', view = 'active') => api.get(`/api/inventory-units?${new URLSearchParams({ ...(q ? { q } : {}), ...(view !== 'active' ? { view } : {}) })}`), create: data => api.post('/api/inventory-units', data), update: data => api.patch('/api/inventory-units', data), verify: data => api.post('/api/inventory-units/verify', data) },
