...
5. **e2e aislado por worktree** (la base y los puertos se comparten):
   ...
   En CI la suite corre en **3 shards balanceados** (`e2e/sharding.json` vía `node scripts/e2e-shards.mjs --shard n`, matriz del workflow; ~103 tests por shard). **Sin reintentos**: un flake se aísla y se corrige la raíz; el reporter deja `test-results/reporte-flaky.md|json` con lo que falló.
