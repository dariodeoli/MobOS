...
| `shared/RucField` (+ `utils/ruc.js`, `lib/demoRuc.js`) | RUC/CI con extractor: botón **adentro del input** (trailing, tooltip, "Consultando…" con spinner; deshabilitado vacío) contra `GET /api/ruc`; en demo resuelve con un resultado simulado marcado; el resultado se aplica solo al confirmar; sin proveedor → carga manual | Alta de cliente, compras, identidad de ficha (CustomerProfile), vendedor y checkout |
...
| `owncoding-ui/BuscadorDispositivo` (+ `lib/dispositivos.js`) | Equipo con dependientes: el modelo manda (búsqueda por nombre o código) y despliega capacidad/color (`tipo="servicio"`) o marca/categoría (`tipo="accesorios"`); cambiar de modelo limpia lo que no aplica. La etiqueta se compone con `etiquetaDispositivo` y se vuelve a abrir con `partesDispositivo`/`varianteDispositivo` | Recepción de la orden de servicio (equipo) y repuestos del caso de garantía |
...
12. **RUC/CI**: `shared/RucField` (botón **Extraer** adentro del input, trailing, contra `GET /api/ruc`): el patrón único vive en `src/utils/ruc.js` (`RUC_RE`, `extraerRuc`, `esRuc`) y la razón social se aplica solo si se confirma. En demo el campo no llama al API: `lib/demoRuc.js` devuelve una razón social ficticia marcada "Simulada en demo" (#234). Si el proveedor no responde, se completa a mano.
