...
test('la cuenta avisa el pago por vencer y el equipo listo para retirar', async ({ page }) => {
  ...
  // Producto propio con stock simple (sin unidades serializadas): el servidor
  // exige los seriales exactos cuando la unidad está serializada.
  const altaProducto = await api(page, '/api/products', {
    method: 'POST',
    body: JSON.stringify({ name: `Producto avisos ${marca}`, sku: `QA240A-${marca}`, pricePyg: 300000, stock: 3 }),
  })
